...
4. 哪些需要用户手动下载或申请？`3` 个高优先源优先需要用户操作，前三名见结论。
5. 哪些已经在本地找到？本地路径验证 found_paths=`4`。
6. 哪些成功转换？light raw-index/conversion sources=`3`；不等同 full benchmark。
7. 哪些能 t+50？本地候选 t+50 sources=`3`；Stage20 actual verified 新增=`0`，仍需 Stage21 episode conversion。
8. 哪些能 t+100？本地候选 t+100 sources=`3`；Stage20 actual verified 新增=`0`，estimated 不等于 actual verified。
...
15. 哪些数据最应该用户下一步提供？`['Stanford Drone Dataset', 'OpenTraj supported datasets', 'full ETH/UCY / EWAP']`。
...
新增 JEPA pretraining 数据源：1
新增 t+50 数据源：0
新增 t+100 数据源：0
是否需要用户提供数据：是
最需要用户提供的数据前三名：['Stanford Drone Dataset', 'OpenTraj supported datasets', 'full ETH/UCY / EWAP']
是否允许 Stage 5C：否
是否允许 SMC：否
