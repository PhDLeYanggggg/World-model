...
def build_real_episodes(
    ...
    episodes = construct_windows(table, source_meta, history_steps, build_horizon, max_agents, max_episodes or (12 if quick else 120))
...
def construct_windows(table: pd.DataFrame, source_meta: Dict, history_steps: int, horizon: int, max_agents: int, max_episodes: int) -> List[Dict]:
    episodes = []
    episode_id = 0
    for scene_id, scene_df in table.groupby("scene_id"):
        ...
        for start_index in range(history_steps - 1, len(frames) - horizon, stride):
            ...
            episode = make_episode(scene_id, scene_df, needed, common, horizon, episode_id, source_meta)
...
def make_episode(scene_id: str, scene_df: pd.DataFrame, frames: List[int], agent_ids: List[str], horizon: int, episode_id: int, source_meta: Dict) -> Dict:
    ...
    for t, frame in enumerate(frames):
        for i, agent_id in enumerate(agent_ids):
            ...
            speed = float(np.linalg.norm([row["vx"], row["vy"]]))
            accel = float(np.linalg.norm([row["ax"], row["ay"]]))
    ...
    return {"meta": meta, "states": states, "agent_ids": agent_ids, "agent_types": [str(lookup.loc[(frames[0], a)]["agent_type"]) for a in agent_ids], "scene": scene_from_bounds(scene_id, bounds)}
...
def scene_from_bounds(scene_id: str, bounds: Dict) -> SceneSpec:
    width = max(1.0, bounds["x_max"] - bounds["x_min"])
    height = max(1.0, bounds["y_max"] - bounds["y_min"])
    return SceneSpec(str(scene_id), width, height, [], {}, [], "real_unknown")
