...
- current_best_deployable: `M3W-Neural v1 composite-tail safe-switch bounded neural dynamics candidate under Stage37/teacher floor (bootstrap+multiseed supported; pending source-level validation before final freeze)`
...
- conclusion: composite-tail safe switching adds a small validation-selected low-risk continuous neural blend outside the repaired teacher switch rows. It is the strongest bootstrap-supported and multiseed-supported fresh candidate and improves the teacher-guided repair on all/t50/t100/hard while preserving easy cases, but it still needs pure-UCY source-level validation before it can be frozen as final M3W-Neural v1.
