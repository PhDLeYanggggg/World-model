...
11. **static-gated full-waypoint repair**：Stage42-J 已完成 policy-level repair，Stage42-K 已完成 fresh checkpoint training，Stage42-L 已修复 fresh checkpoint 的 t50 ADE 负号；下一步是把 Stage42-J 的 stronger policy expert selection 蒸馏进 horizon/domain-aware checkpoint。
...
1. **distill Stage42-J policy gate into fresh checkpoint**：Stage42-L 已把 t50 修正为正，但仍弱于 Stage42-J；下一步要让 checkpoint 学到 J 的 domain/horizon expert selection，而不是只做单一 learned static gate。
