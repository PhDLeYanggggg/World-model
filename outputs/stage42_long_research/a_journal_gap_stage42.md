...
- Policy distillation: Stage42-M shows that distilling only slice-level static alpha can improve FDE t50 but harms ADE t50. Stage42-N shows that row-level alpha/gain/harm supervision improves all/hard but still harms ADE t50. Static-gate distillation is therefore not enough for a deployable checkpoint without an explicit t50 gain/harm selector.
...
1. Build an explicit row-level gain/harm/switchability selector head for full-waypoint t+50, because Stage42-N shows that row-level alpha supervision alone still switches harmful t50 rows.
