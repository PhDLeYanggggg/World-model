...
## CREATE 连接已实测恢复，限定只读（2026-09-23）

按用户要求向 `simulation model` 取得交接后，使用其提供的既有身份和严格 SSH
参数完成一次 hostname 与三次串行调度器查询，全部退出 0。这是本轮真实连接，
不是仅复用历史配置。旧章节的认证失败与“账户未知”是历史记录，不再是当前结论。

完整交接、实际参数和受保护对象保存在 Git 忽略的本地文件：
`data/stage_cvpr2027_experiments/create_handoff_20260923/observations.json`。
不要把账户、密钥路径或完整内部调度器输出复制到公开报告；不读取密钥内容。
公开说明见 [只读交接报告](create_readonly_handoff_20260923.md)。

必须保留 BatchMode、IdentitiesOnly、UseKeychain、ConnectTimeout=10、
StrictHostKeyChecking；每次查询由父进程施加 60 秒超时。没有经过核验的 alias
或 ControlMaster，不假定可以复用或关闭别人的会话。保持串行、有界、低频；
超时不能解释为训练终止。禁止改 key/agent/Keychain/VPN/转发/持久配置或权限。

simulation 两个训练任务仍在运行，四个元素因维护等待，CPU 审查等待依赖。
严禁取消、重提、缩时、迁移、清缓存、安装包或修改其工程/环境/日志/结果。
只读查询确认 biomed 标准时限 48 小时，9 月 24 日 07:00 至 25 日 23:59:59
有全节点维护预约（英国时间）。不能据此假定有空闲 GPU、通用八卡配额或 M3W
提交权限；long_gpu 不可擅自使用，也不采用 interruptible 规避限制。

M3W 的远程目录仍未确认，不能把 simulation 目录当作 M3W 目录；本轮没有泛扫
远程 home 或任何项目目录。没有上传、建目录、提交/取消/重启任务或科学计算。
登录节点只用于轻量管理，不运行训练/科学评价。下一步只对明确获准的 M3W 路径
核查已有资产，再依既定科研协议决定是否需要计算资源。
...
36,864,000次抽样，旧预测器与对照复用但核验hash。新检查点重放527,268个分数。
39项未修改的针对性测试已通过，不等于全仓库测试或独立研究复现。

严格策略ADE改善从3.72892%到4.09764%，差值0.36871个百分点，CI为
[0.17316,0.60302]。但deathCircle与gates的easy平均退化为2.93332%和2.15061%，
逐场景/种子保护仍失败。旧选中区域拟合改善，新选中区域仍低估伤害；不部署。
...
CPU4/inter-op1/workers0，成本头实际拟合133.80秒，不含此前上游预测器训练、
读取和验证。初次执行/恢复见`conditional_cost_v1/execution_notes.md`。原始关闭
角色未打开，独立风险校准与最终确认未运行，Stage5C/SMC仍关闭。

## 等容量、等预算损失对照：训练与核验完成

登记提交`06f9a635`先于训练。24个新native/fraction头完成288,000次更新、
73,728,000次抽样，12个中间损失头是旧产物的hash核验复用。36个检查点、
1,581,804个分数重放一致，另写算术核验108组策略、864项场景归约及监督标签。
48项针对性测试通过；不是全仓库测试通过，也不是独立研究团队复现。

相同预算下，严格策略ADE改善分别为3.06719%、1.65767%、3.72892%。中间损失
相对fraction提升2.07125个百分点，区间不跨0；相对native提升0.66173个百分点，
区间[-0.27228,2.18556]跨0。预定规则要求两项都通过，因此主贡献检验失败。
deathCircle简单样本仍退化2.94724%；不部署，也不把普通对照失败当创新证明。

已完成产物只需核验，不要重训或重跑pilot：
...
CPU4/inter-op1/workers0，真实Torch头拟合245.01秒，不含上游预测器和I/O核验。
首次执行与恢复说明见`cost_budget_matched_v1/execution_notes.md`。原始关闭角色
没有打开，独立场景校准仍缺失，Stage5C/SMC均不执行。

## 容量与训练时长：开发主比较通过，仍不部署

登记提交`621a6eb2`先于训练。12条新宽网络路径与12条窄网络续训路径均完成，
新增252,000次更新、64,512,000次抽样。36个新增端点与12个旧端点全部重放，
2,109,072个分数一致。另写算术核验144组策略、1,152项场景归约及1,581,804个监督行。
50项针对性测试通过，非全仓库测试成功。全仓库存在已知会改写其他数据湖/报告状态的测试。

宽度128/12,000步严格策略ADE改善3.72892%，相对旧fraction对照增加2.11962个百分点，
CI为[0.51680,4.53486]，固定开发主比较通过。整体easy改善0.81395%，但deathCircle
easy退化2.94724%，三个种子均超过2%。被选样本伤害仍被低估，缺失未来不能视为安全。
因此不部署，也不能称独立泛化成功。下一步让原生/fraction损失获得相同容量与预算再比较。
...
首次执行、pilot与resume见`cost_capacity_v1/execution_notes.md`。不要重跑已完成pilot。
CPU4/inter-op1/workers0；真实Torch成本头训练耗时约247.81秒，不包括读取、核验、
继承的旧训练及此前7.75小时预测器训练。原始保留角色不打开，Stage5C/SMC不执行。

## 固定中间损失：训练完成，主比较失败

登记提交`5a9d02ab`先于训练。12个预测头、36,000次更新、9,216,000次抽样均已完成，
没有未知监督样本被抽取。全部527,268个预测分数重放一致，另写算术核验36组策略、
288项场景归约和1,581,804个重复监督行。99项针对性测试通过。

严格策略ADE改善1.09819%，相对旧fraction策略差-0.51112个百分点，CI跨0，主比较失败。
整体easy退化0.54512%，但deathCircle/seed29达3.20327%，必须同时报告。匹配切换数量时
排序略有改善，但easy退化7.49659%，不部署。高分歧排序得到修复，低风险小收益机会却被
丢弃；下一步检查拟合容量与收敛，不继续搜索指数或测试阈值。独立校准和最终确认仍未运行。

以下保留最初执行顺序。对于已完成产物应运行verify，而不是重跑pilot。

成本拟合诊断已两次得到相同结果：高分歧样本的误排序在训练内就存在，不只是迁移问题。
下一轮仅把原生成本误差平方的分歧权重改为固定指数1；原先对照是指数0和2。
不修改模型、采样或阈值。12个新头仍用4个研究场景、3个种子、每头3000次更新。
最初23项针对性测试和904个依赖绑定预检通过，正式训练在登记时尚未开始。
...
`1a9d26ed`并训练36个新成本头，共108,000次更新；登记先于新结果读取。缓存恢复保留
7个完整产物，没有重训预测器。两套核验程序通过，91项针对性测试通过。

结果：主策略相对CV的ADE改善1.60931%，easy误差下降1.78803%，已观测完整零CV样本
没有新增损伤。但相对旧冻结策略的差值为-1.71680个百分点，区间跨0，主比较失败。
相同切换数量下也没有证明排序改善。所有结果仍来自4个用于研究设计的场景，不是独立
最终测试。没有部署、没有进入Stage5C或启用SMC。
...
上述pilot仅适用于新实验，已完成模型不能倒退到100步重跑。现有产物使用verify核验；
中断训练用resume。同群体诊断属于读取结果后的取证，不更改阈值或模型，也不是因果证明。
完整数值与失败分类见`eqmotion_cost_refit_v1/conclusions.md`。
