...
Latest continuation: the [experiment contract](experiment_contract/implementation_and_limits.md) binds scientific choices, recording roles and recursive fitted-artifact provenance. It freezes a calibration family and a confirmation run before their respective reader access, with identity-preserving resume. Twenty-five new contract tests and the 48 related prior checks pass (73 total). The current real-asset draft remains unapproved/unassigned and correctly refuses a normal run with exit 2. All nine current recordings are historically exposed development material; no untouched holdout was created. This interface is not yet wired into every legacy trainer and cannot prove truthful declarations or IID.

Verification: previous full suite 1,870 passed / 1 failed in 73 minutes; the latest focused suite for the joint-intervention/reader changes is 48 passed. The independently reproduced full-suite failure is in the pre-existing data-lake fixture's missing manual handoff. Legacy test-written reports/state were quarantined and recovered; no replay was promoted to new research evidence. See [verification and isolation record](verification_record.md). The full suite remains non-hermetic and was not rerun for this scoped change.
...
Next: resolve the pending protocol choice and CREATE project path, inspect the longer University recording's provenance, then fit a clean fixed predictor and split-bound gain/harm heads for the implemented control-arm comparison. Joint energies alone are not novel; see the [focused prior-work review](joint_intervention/related_work_constraints.md). Do not train from rejected legacy caches. See [runbook](local_create_runbook_zh.md), [latest causal checks](joint_intervention/causal_recording_checks.md), and the three `runtime_*.json` files for executable evidence. Runtime input bytes remain unchanged after the reversible-coordinate refactor.

Independent work while scientific choices/access remain pending: review local GC/HERMES/Wild-Track/CITR/VRU source identity, original use conditions, canonical scene support and prior project use. Do not inspect model performance to pick an apparently favorable confirmation domain. Presence of files or a clean metadata gate is insufficient to designate an independent benchmark.
