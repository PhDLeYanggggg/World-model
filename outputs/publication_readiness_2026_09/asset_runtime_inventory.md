...
Latest continuation: the [development evaluator](development_evaluation/implementation_and_limits.md) connects verified predictors/cost heads to five matched-forecast control arms, development-only selection and scene-block reporting. It decides for all past-supported agents before reading labels, reports actual intervention rates, and does not synthesize missing endpoint labels. Different native grids stay separate. Seventeen new tests bring the combined focused set to **119 passed in 16.80 s**. The synthetic end-to-end candidate was rejected in favor of its floor; this is negative engineering-fixture behavior, not a real-data result. A new 23-query / 274-agent real-input check used random forecasts and constant synthetic scores: all five controls were invariant to future corruption, zero labels read. Scientific approval and real fitting/evaluation are still pending; independent calibration/confirmation, strong public controls and statistical evidence remain incomplete.

The preceding [causal supervised backend](supervised_backend/implementation_and_limits.md) connects the verified recording reader to a fixed-budget Torch forecaster and out-of-fold realized benefit/harm learning. It does not use old teacher outputs. Atomic checkpoints include optimizer, sampler and RNG; OOF fold caches are verified on resume. CPU and MPS synthetic recovery passed, and its earlier 24 real-input queries / 345 agents remained unchanged under future-position corruption with zero label calls. Its own combined set was 102 passed in 11.65 s. No real forecast training, development selection, calibration or confirmation has run through this backend. The MLP cost interface is implemented, while the trained synthetic cost control is ridge.
