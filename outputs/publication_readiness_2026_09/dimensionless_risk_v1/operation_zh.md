...
.venv-pytorch/bin/python scripts/plot_m3w_dimensionless_risk.py
...
这个版本只改变风险头特征，未接入 canonical-prefix 新接口：后者改变低速
轨迹归一化。原 v1 的 EqMotion 数值失败已被单独的 precision-v2 固定精度
探针修复，但两版记录均保留，v2 也没有混入本轮训练。把两者一起改会破坏
单因素对照；数值探针通过不等于预测误差改善。
