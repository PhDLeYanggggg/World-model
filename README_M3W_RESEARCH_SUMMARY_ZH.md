...
Stage42-BD 把 Stage42-BC 的 acquisition plan 往本机实际路径推进了一步：扫描本地 OpenTraj / ETH / UCY / TrajNet 目录后，发现 4 个尚未纳入当前 t100 source-CV 的 novel t100-capable candidate files，包括 `ETH/seq_eth/biwi_eth_10fps.txt`、`UCY/students01/students001.txt`、`UCY/students03/obsmat_px.txt` 和 `UCY/students03/students003.txt`，估计可提供约 `6257` 个 novel t100 windows。
...
Stage42-BG 是对 BF 的进一步推进：它不再只做 baseline audit，而是在 train / validation source 上选择 protected baseline-family policy，并在 holdout source 上只评估一次。UCY local t100 source-CV 结果为 positive 且 easy-safe；但 ETH_UCY 仍只有 1 个 t100-capable source，TrajNet 不在这批新本地候选里，因此 global t100 success 仍然 blocked。这个结论不能写成“t100 已修复”，只能写成“UCY local t100 source-CV support positive”。
...
18. **Stage42-AX 固化 AW repaired protocol 的 robustness，同时明确 t100 limitation。**
   Stage42-AX 不重训、不调 threshold，而是读取 AW/AV 已验证结果做 cached_verified robustness audit。结果 14/14 gates：global bootstrap 下界 all `+35.31%`、t50 `+28.54%`、t100 raw-frame diagnostic `+20.29%`、hard/failure `+33.52%`，easy degradation CI high 为负；TrajNet 和 UCY 两个 domains 均 positive。限制也很明确：horizon=100 的 row-level easy degradation 是 `2.396%`，超过 2% safety threshold，所以 uniform horizon success 仍不允许，t100 只能写 raw-frame diagnostic with easy-safety limitation。

19. **Stage42-AY 用 validation-only strict t100 guard 修复 AX 的 h100 easy-safety weak slice。**
   Stage42-AY 重新计算 AW validation-best `family_baseline_rel_only` 的 full-waypoint arrays，不用 test metrics 选 threshold；它只用 validation t100 easy-safety 判断是否回退 floor。`TrajNet|100` 因 validation easy degradation `1.52%` 高于 stricter non-harm threshold `0.0` 被回退；`UCY|100` 保留。结果 17/17 gates：h100 easy degradation 从 `2.396%` 修到 `-0.650%`，h100 easy CI high `0.983%`，global all `+30.55%`，t50 `+28.97%`，t100 raw-frame diagnostic `+6.78%`，hard/failure `+27.98%`。这是更安全的 repaired-policy candidate，但由于它是 AX 后的修复，强 paper claim 还需要未来独立 holdout / source-level confirmation。
