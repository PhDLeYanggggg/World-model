...
结果来源：`cached_verified_summary`，汇总既有 Stage18 到 Stage42-HU 报告、gate、model/data card、README 与 `research_state.json`。
...
- t100 raw-frame：Stage42-HR/HS 增加了 validation-only t100 easy guard 与冻结复放证据，修复了 HQ 暴露的 t100 easy harm；Stage42-HT 又把该 frozen policy 做成可调用 runtime API；Stage42-HU 进一步审计出真实 row-level batch replay 仍缺 `candidate/floor/selected` rollout arrays，因此 HT 只能写 runtime API + smoke replay，不能写成真实 batch replay。t100 仍只能写 raw-frame diagnostic，不能写 seconds-level。
...
| Stage42-HR/HS/HT/HU | t100 easy guard + frozen replay + runtime API + replay sufficiency audit | HR：all +27.72%，t50 +26.99%，t100 raw +6.79%，hard +25.93%，t100 easy degradation -0.31%，gate 23/23；HS：policy hash/replay 固化，gate 27/27；HT：runtime API gate 19/19；HU：runtime API ready、frozen policy ready，但 real batch replay `not_run`，blocker=`missing_row_level_candidate_floor_selected_arrays` | 修复 t100 easy harm，并可调用 smoke replay；但真实 row-level batch replay 仍未完成，t100 仍是 raw-frame diagnostic，不是 seconds-level |
...
#### Stage42-HR/HS/HT/HU t100 easy guard, runtime policy and replay sufficiency
...
- 最新 blocker：Stage42-HU 明确指出 HR/HS/HT artifacts 不包含真实 row-level `candidate_xy_predicted_rollout`、`floor_xy_train_horizon_causal_rollout`、`selected_xy` / row-id alignment arrays；所以 HT 是 callable runtime API + smoke replay，不是 full row-level batch replay。下一步若要写 deployment replay，需要先生成或恢复这些 row-level arrays。
...
| t100 raw-frame diagnostic safety | Stage42-HR/HS/HT/HU group-consistency t100 easy guard runtime policy + replay sufficiency audit | 候选但 batch replay blocked | 修 t100 easy harm并提供 runtime API；HU 证明真实 row-level batch replay 还缺 arrays；不能写 seconds-level |
...
- Stage42-HR/HS/HT/HU t100 easy guard：all +27.72%，t50 +26.99%，t100 raw diagnostic +6.79%，hard +25.93%，t100 easy degradation -0.31%，frozen replay exact，runtime policy gate 19/19；HU gate 17/17，但真实 row-level batch replay 仍 `not_run`，缺 candidate/floor/selected rollout arrays。
