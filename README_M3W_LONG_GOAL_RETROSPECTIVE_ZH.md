...
| Stage42-BS | UCY_zara family-only source-CV：rows 51,544，t50 rows 12,750，t50 macro +24.72%，t50 min +15.10%，hard +6.72%，easy max 1.24%，gates 14/14 | `UCY_zara` policy/model blocker 修复；`ETH_seq` 和 `UCY_students` 仍需同族源支持 |
| Stage42-BT | ETH-Person XML h50 technical dry-run：h50 mean +41.12%，3/5 folds safe-positive，但 `ETH_seq_eth` holdout improvement = 0，gates 13/13 | ETH-Person 技术信号不能安全修复 `ETH_seq` calibrated t50；ETH_seq blocker 仍在 |
| Stage42-BU | UCY_students 本地源审计：9 个候选路径、2 个 independent t50-capable sources（students001/students003），还缺 1 个，gates 14/14 | `UCY_students` blocker 从“缺 2 个源”缩小到“还缺 1 个真正 t50-capable independent source” |
| Stage42-BV | source acquisition / blocker matrix：5 个 active blockers，gates 16/16；没有自动下载、没有 registry-only 转成功、没有 metric/seconds overclaim | 把 ETH_seq、UCY_students、TrajNet long-track、ETH_UCY t100、global metric/seconds 的下一步动作固化成可执行清单 |
...
### 4.8 calibrated subset t50 为什么从 non-harm 进入局部修复
...
- Stage42-BS 对唯一不需要新数据的 `UCY_zara` blocker 做 family-specific t50 policy，修复成功：三折 t50 全正、easy 低于 2%。
...
- `UCY_zara` 有 3 个 sources；Stage42-BS 证明在更保守的 validation-only t50 switch-rate guard 下可以做到 positive safe t50。
- ETH-Person XML 对 ETH-Person holdouts 有 h50 技术正信号，但 Stage42-BT 证明它不能安全迁移到 `ETH_seq_eth` holdout；因此不能把 ETH-Person 技术信号写成 ETH_seq 修复。
- Stage42-BU 证明本地 `students001` 是新增 t50-capable 同族源，`students003` 是已有 source，`students002` 太短；所以 `UCY_students` 还差 1 个 independent t50-capable students-family source。
- Stage42-BV 证明当前剩下的不是“再写一个报告”问题，而是 5 个明确 active blockers：ETH_seq t50 source support、UCY_students t50 source support、TrajNet raw long t100 source support、ETH_UCY global t100 source support、global metric/seconds claim。下一步必须先补合法源或 terms，再重跑 conversion / no-leakage / source-CV。

结论：

```text
calibrated subset 目前可以写 limited positive all/hard + t50 non-harm。
UCY_zara 可以写 source-family-specific positive t50 repair。
ETH_seq / UCY_students 仍不能写 positive calibrated t50 transfer；其中 UCY_students blocker 已被缩小为还缺 1 个 independent t50-capable source。
...
### 下一步 1：ETH_seq / UCY_students source support

Stage42-BR 已说明：

```text
ETH_seq needs one more same-family calibrated support source.
UCY_students needs one more independent t50-capable same-family source after Stage42-BU.
```

Stage42-BS 已经修复了不需要新数据的 `UCY_zara` policy/model blocker。剩余 calibrated t50 blocker 主要是源支持不足：

- `ETH_seq`：可在 ETH-Person XML terms 确认后尝试补同族 source。
- `UCY_students`：Stage42-BU 已找到 `students001` 作为新增 t50-capable source，但 `students002` 太短；还需要 1 个额外合法同族 long-track source。
- 如果没有新增同族源，BQ 的 fallback/non-harm 守门仍是正确部署边界。
- Stage42-BT 已确认：仅靠当前 ETH-Person XML technical dry-run 不能安全修复 `ETH_seq_eth` calibrated t50。
...
calibrated-subset t50:
  global non-harm yes
  UCY_zara positive source-family-specific repair yes
  ETH_seq positive transfer no, still source-support blocked
  UCY_students positive transfer no, still needs one independent t50-capable source
  ETH-Person XML h50 technical signal does not safely repair ETH_seq_eth
