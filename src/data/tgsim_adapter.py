...
ALIASES = {
    ...
    "x": ["x", "x_m", "global_x", "center_x", "xcenter", "location_x", "pos_x", "x_position", "xloc_kf", "xloc"],
    "y": ["y", "y_m", "global_y", "center_y", "ycenter", "location_y", "pos_y", "y_position", "yloc_kf", "yloc"],
    ...
    "vx": ["vx", "v_x", "velocity_x", "x_velocity", "speed_kf_x", "speed_x"],
    "vy": ["vy", "v_y", "velocity_y", "y_velocity", "speed_kf_y", "speed_y"],
    "ax": ["ax", "a_x", "acceleration_x", "x_acceleration", "acceleration_kf_x", "accel_x"],
    "ay": ["ay", "a_y", "acceleration_y", "y_acceleration", "acceleration_kf_y", "accel_y"],
    "width": ["width", "width_m", "object_width", "width_smoothed"],
    "length": ["length", "length_m", "object_length", "length_smoothed"],
    "agent_type": ["type", "agent_type", "road_user_type", "object_type", "class", "type_most_common"],
}
...
def normalize_tgsim_chunk(chunk: pd.DataFrame, column_map: TGSIMColumnMap | None = None, pixels_to_meters: bool = False) -> pd.DataFrame:
    ...
    out["ax_mps2"] = pd.to_numeric(chunk[cmap.ax], errors="coerce") if cmap.ax else np.nan
    out["ay_mps2"] = pd.to_numeric(chunk[cmap.ay], errors="coerce") if cmap.ay else np.nan
...
def fill_missing_kinematics(data: pd.DataFrame) -> pd.DataFrame:
    ...
    dvx = out.groupby("agent_id")["vx_mps"].diff()
    dvy = out.groupby("agent_id")["vy_mps"].diff()
    out["ax_mps2"] = out["ax_mps2"].fillna(dvx / dt.replace(0, np.nan))
    out["ay_mps2"] = out["ay_mps2"].fillna(dvy / dt.replace(0, np.nan))
    ...
    return out.fillna({"vx_mps": 0.0, "vy_mps": 0.0, "ax_mps2": 0.0, "ay_mps2": 0.0, "speed_mps": 0.0, "acceleration_mps2": 0.0})
