"""Training-only continuation of verified source dynamics checkpoints."""
import copy
import math
import os
import time
...
def learning_rate(schedule, step, config):
    """Rate used for the update from global step to step + 1."""
    start, end = config['start_step'], config['updates']
    if schedule not in SCHEDULES or not start <= step < end or end - start < 2:
        raise ValueError('Registered schedule and continuation step required')
    fraction = (step - start) / (end - start - 1)
    multiplier = 1.0 if schedule == 'constant' else (
        config['minimum_lr_ratio'] + (1 - config['minimum_lr_ratio'])
        * 0.5 * (1 + math.cos(math.pi * fraction)))
...
def continue_fit(model, inputs, targets, train_ids, weights, *, parent,
                 schedule, config, identity, checkpoint, heartbeat,
                 snapshot, stop_at=None):
    ids = np.asarray(train_ids, dtype=int)
    weights = torch.as_tensor(weights, dtype=torch.float64)
    if (schedule not in SCHEDULES or len(np.unique(ids)) != len(ids)
    ...
    if (parent['step'] != config['start_step'] or parent['arm'] != 'past_rgb'
            or parent['objective'] != 'ade' or parent['normalizer'] <= 0):
        raise ValueError('Verified ADE/RGB parent checkpoint required')
    ...
    if checkpoint.exists():
        ...
        if state['identity'] != identity or state['config'] != config or state['schedule'] != schedule:
    ...
    def payload():
        return dict(identity=identity, config=config, schedule=schedule,
                    normalizer=parent['normalizer'], arm='past_rgb', objective='ade',
    ...
    try:
        while step < limit:
            ...
            prediction = restore(model(*features, 'past_rgb'), *frame)
