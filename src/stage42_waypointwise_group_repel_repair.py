...
REPORT_JSON = OUT_DIR / "waypointwise_group_repel_repair_stage42.json"
REPORT_MD = OUT_DIR / "waypointwise_group_repel_repair_stage42.md"
GATE_MD = OUT_DIR / "stage42_stage_fa_gate.md"

DI_JSON = OUT_DIR / "group_consistency_full_waypoint_repair_stage42.json"
EZ_JSON = OUT_DIR / "temporal_group_repel_repair_stage42.json"
...
def _smooth_offsets(offsets: np.ndarray) -> np.ndarray:
    if len(offsets) < 3:
        return offsets
    out = offsets.copy()
    out[1:-1] = 0.25 * offsets[:-2] + 0.50 * offsets[1:-1] + 0.25 * offsets[2:]
    return out


def _repel_waypointwise_rows(
    xy: np.ndarray,
    switch: np.ndarray,
    ...
    while start < len(order):
        ...
        if len(unique_rows) > 1 and np.any(switch[unique_rows]):
            ...
            for i, row in enumerate(unique_rows):
                ...
                cap = float(cap_scale) * sep
                offsets = np.zeros_like(pts[i], dtype=np.float64)
                for t in range(pts.shape[1]):
                    diff = pts[i, t][None, :] - pts[:, t, :]
                    ...
                    if norm < EPS:
                        ...
                        norm = 1.0
                    mag = min((sep - min_d) * float(strength) * float(weights[t]), cap)
                    offsets[t] = direction / norm * mag
                if smooth:
                    offsets = _smooth_offsets(offsets)
                repaired[row] = repaired[row] + offsets.astype(np.float32)
        start = end
    return repaired
...
def _candidate_grid() -> list[dict[str, Any]]:
    ...
    for min_sep in [0.05, 0.08, 0.12]:
        for strength in [0.20, 0.40, 0.60]:
            for temporal_kind in ["uniform", "tail", "sqrt_tail", "bell"]:
                for smooth in [False, True]:
                    rows.append(
                        {
                            "mode": "waypointwise_repel",
                            "min_sep": min_sep,
                            "strength": strength,
...
def _repair_subset_waypointwise(
    ids: np.ndarray,
    ...
    unsafe = switch & np.isfinite(base_min) & np.isfinite(floor_min) & (base_min < min_sep) & (base_min < floor_min)
    del pred_min
    weights = ez._temporal_weights(str(candidate["temporal_kind"]), selected_xy.shape[1], float(candidate.get("gamma", 1.0)))
    selected_xy = _repel_waypointwise_rows(
        selected_xy,
        switch,
        keys,
        normalizer,
        ...
        weights=weights,
        smooth=bool(candidate.get("smooth", False)),
        cap_scale=float(candidate.get("cap_scale", 0.75)),
    )
...
def _selection_score(metric: Mapping[str, Any], diagnostics: Mapping[str, Any]) -> float:
    ...
    return (
        ...
        - 10.0 * max(0.0, near_delta)
        - 0.01 * float(metric["switch_rate"])
    )


def _evaluate_waypointwise_repairs(
    data: Mapping[str, np.ndarray],
    split: np.ndarray,
    labels: Mapping[str, np.ndarray],
    ...
    for candidate in _candidate_grid():
        val = _repair_subset_waypointwise(val_ids, candidate, data, labels, floor_xy, pred_xy, base_xy, base_switch, group_key)
        ...
        if score > best_score:
            ...
            best = row
    if best is None:
        raise RuntimeError("No Stage42-FA waypoint-wise repair candidate evaluated.")
    test = _repair_subset_waypointwise(test_ids, best["candidate"], data, labels, floor_xy, pred_xy, base_xy, base_switch, group_key)
    ...
    bootstrap = {
        "all": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], np.ones(len(test_ids), dtype=bool), seed=43001),
        "t50": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], h == 50, seed=43002),
        "t100_raw_frame_diagnostic": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], h == 100, seed=43003),
        "hard_failure": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], hard_failure, seed=43004),
        "easy_degradation": di._bootstrap_ci_subset(test["floor_ade"], test["selected_ade"], easy, seed=43005),
    }
    by_domain = {
...
def _compare_to_prior(metric: Mapping[str, Any], diagnostics: Mapping[str, Any]) -> dict[str, Any]:
    di_cmp = ez._compare_to_stage42_di(metric, diagnostics)
    ez_payload = read_json(EZ_JSON, {})
    ez_metric = ez_payload.get("repair", {}).get("test", {}).get("metric_vs_floor", {})
    ez_diag = ez_payload.get("repair", {}).get("test", {}).get("diagnostics", {})
    if ez_metric:
        delta_ez = {
            "all_improvement": float(metric.get("all_improvement", 0.0)) - float(ez_metric.get("all_improvement", 0.0)),
            "t50_improvement": float(metric.get("t50_improvement", 0.0)) - float(ez_metric.get("t50_improvement", 0.0)),
            "t100_raw_frame_diagnostic_improvement": float(metric.get("t100_raw_frame_diagnostic_improvement", 0.0))
            - float(ez_metric.get("t100_raw_frame_diagnostic_improvement", 0.0)),
            "hard_failure_improvement": float(metric.get("hard_failure_improvement", 0.0)) - float(ez_metric.get("hard_failure_improvement", 0.0)),
            "easy_degradation": float(metric.get("easy_degradation", 0.0)) - float(ez_metric.get("easy_degradation", 0.0)),
        }
        near_delta_ez = float(diagnostics.get("final_near_005", 0.0)) - float(ez_diag.get("final_near_005", 0.0))
    else:
        delta_ez = {k: None for k in ["all_improvement", "t50_improvement", "t100_raw_frame_diagnostic_improvement", "hard_failure_improvement", "easy_degradation"]}
        near_delta_ez = None
    di_cmp["stage42_ez_metric"] = ez_metric
    di_cmp["stage42_ez_diagnostics"] = ez_diag
    di_cmp["delta_vs_stage42_ez"] = delta_ez
    di_cmp["near_delta_vs_stage42_ez"] = near_delta_ez
    return di_cmp
...
def _build_payload() -> dict[str, Any]:
    ...
    eval_result = _evaluate_waypointwise_repairs(data, split, labels, floor, am_candidate, group_key)
    metric = eval_result["test"]["metric_vs_floor"]
    diagnostics = eval_result["test"]["diagnostics"]
    comparison = _compare_to_prior(metric, diagnostics)
    ...
    result: dict[str, Any] = {
        "source": SOURCE,
        "stage": "Stage42-FA waypoint-wise group-repel repair",
        ...
        "repair_family": {
            "source": "fresh_stage42_fa_waypointwise_repel_grid",
            "candidate_count": len(_candidate_grid()),
            "temporal_shapes": sorted(set(c["temporal_kind"] for c in _candidate_grid())),
            "smooth_options": sorted(set(bool(c["smooth"]) for c in _candidate_grid())),
            "baseline_reference": "Stage42-DI uniform same-offset repel_unsafe and Stage42-EZ temporal single-direction repel",
            "uses_future_inputs": False,
        },
        "stage42_am_rebuilt": {
            "source": "fresh_rebuild_for_fa",
        ...
        "deployment_decision": {
            "promote_waypointwise_group_repel": bool(promotes),
            "decision": "promote_stage42_fa_waypointwise_group_repel_repair"
            if promotes
            else "waypointwise_group_repel_not_enough_keep_stage42_di_or_cq_floor",
    ...
    result["stage42_fa_gate"] = _gate(result)
    return result
...
def _gate(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    delta_di = result["comparison_to_prior"]["delta_vs_stage42_di"]
    near_delta = result["comparison_to_prior"]["near_delta_vs_stage42_di"]
    ...
    gates = {
        ...
        "waypointwise_repair_family_built": result["repair_family"]["candidate_count"] >= 40,
        "validation_selected_waypointwise_repair": result["repair"]["selected"]["val_score"] != 0.0 and no_leak["test_threshold_tuning"] is False,
    ...
    total = len(gates)
    if passed == total:
        verdict = "stage42_fa_waypointwise_group_repel_repair_pass_promotable"
        ...
        verdict = "stage42_fa_waypointwise_group_repel_repair_partial"
    return {"gates": gates, "passed": passed, "total": total, "verdict": verdict}


def _render_report(result: Mapping[str, Any]) -> list[str]:
    gate = result["stage42_fa_gate"]
    selected = result["repair"]["selected"]
    metric = result["repair"]["test"]["metric_vs_floor"]
    diag = result["repair"]["test"]["diagnostics"]
    base = result["repair"]["baseline_stage42_am_on_test"]
    delta_di = result["comparison_to_prior"]["delta_vs_stage42_di"]
    delta_ez = result["comparison_to_prior"]["delta_vs_stage42_ez"]
    lines = [
        ...
        *[f"- {fact}" for fact in CURRENT_FACTS],
        ...
        f"- smooth_options: `{result['repair_family']['smooth_options']}`",
        "",
        ...
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        f"| Stage42-AM rebuilt floor-protected source-level | {ez._pct(base['all_improvement'])} | {ez._pct(base['t50_improvement'])} | {ez._pct(base['t100_raw_frame_diagnostic_improvement'])} | {ez._pct(base['hard_failure_improvement'])} | {ez._pct(base['easy_degradation'])} | {ez._pct(base['switch_rate'])} | {ez._pct(diag['base_near_005'])} |",
        f"| Stage42-FA waypoint-wise group-repel | {ez._pct(metric['all_improvement'])} | {ez._pct(metric['t50_improvement'])} | {ez._pct(metric['t100_raw_frame_diagnostic_improvement'])} | {ez._pct(metric['hard_failure_improvement'])} | {ez._pct(metric['easy_degradation'])} | {ez._pct(metric['switch_rate'])} | {ez._pct(diag['final_near_005'])} |",
        "",
        "## Delta vs Stage42-DI / Stage42-EZ",
        "",
        f"- delta_vs_stage42_di all/t50/t100raw/hard/easy: `{ez._pct(delta_di['all_improvement'])}` / `{ez._pct(delta_di['t50_improvement'])}` / `{ez._pct(delta_di['t100_raw_frame_diagnostic_improvement'])}` / `{ez._pct(delta_di['hard_failure_improvement'])}` / `{ez._pct(delta_di['easy_degradation'])}`",
        f"- delta_vs_stage42_ez all/t50/t100raw/hard/easy: `{ez._pct(delta_ez['all_improvement'])}` / `{ez._pct(delta_ez['t50_improvement'])}` / `{ez._pct(delta_ez['t100_raw_frame_diagnostic_improvement'])}` / `{ez._pct(delta_ez['hard_failure_improvement'])}` / `{ez._pct(delta_ez['easy_degradation'])}`",
        f"- near_delta_vs_stage42_di: `{ez._pct(result['comparison_to_prior']['near_delta_vs_stage42_di'])}`",
        f"- near_delta_vs_stage42_ez: `{ez._pct(result['comparison_to_prior']['near_delta_vs_stage42_ez'])}`",
        "",
        "## Bootstrap CI",
        "",
...
def _refresh_lines(result: Mapping[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"- near@0.05 base/final: `{ez._pct(diag['base_near_005'])}` / `{ez._pct(diag['final_near_005'])}`.",
        f"- decision: `{result['deployment_decision']['decision']}`.",
...
def _refresh_readmes(result: Mapping[str, Any]) -> None:
    ...
    for path in [README_RESULTS, M3W_README, WORK_SUMMARY]:
        _replace_section(path, "STAGE42_FA_WAYPOINTWISE_GROUP_REPEL_REPAIR", lines)
...
def _refresh_research_state(result: Mapping[str, Any]) -> None:
    ...
    state["current_stage"] = "Stage42-FA waypoint-wise group-repel repair"
    state["current_verdict"] = result["stage42_fa_gate"]["verdict"]
    state["stage42_fa_waypointwise_group_repel_repair"] = {
        "source": result["source"],
        "report": str(REPORT_MD),
        "gate": str(GATE_MD),
        "verdict": result["stage42_fa_gate"]["verdict"],
        "gates": f"{result['stage42_fa_gate']['passed']}/{result['stage42_fa_gate']['total']}",
        ...
        "comparison_to_prior": result["comparison_to_prior"],
        "claim_boundary": result["claim_boundary"],
        "conclusion": "Stage42-FA tests per-waypoint neighbor-geometry repel after Stage42-EZ temporal single-direction repair failed proximity promotion.",
    }
    write_json(RESEARCH_STATE, ez._jsonable(state))


def run_stage42_waypointwise_group_repel_repair() -> dict[str, Any]:
