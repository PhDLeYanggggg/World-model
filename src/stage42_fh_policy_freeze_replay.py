...
from src import stage42_ucy_supported_fe_composer as fh
...
REPORT_JSON = OUT_DIR / "fh_policy_freeze_replay_stage42.json"
REPORT_MD = OUT_DIR / "fh_policy_freeze_replay_stage42.md"
POLICY_JSON = OUT_DIR / "frozen_ucy_supported_fe_composer_policy_stage42.json"
POLICY_MD = OUT_DIR / "frozen_ucy_supported_fe_composer_policy_stage42.md"
GATE_MD = OUT_DIR / "stage42_stage_fi_gate.md"
...
def _by_domain(data: Mapping[str, np.ndarray], test_ids: np.ndarray, selected_ade: np.ndarray, floor_ade: np.ndarray, switch: np.ndarray) -> dict[str, Any]:
    domains = data["dataset"][test_ids].astype(str)
    return {
        domain: di._metric_subset(selected_ade[domains == domain], floor_ade[domains == domain], data, test_ids[domains == domain], switch[domains == domain])
...
def _replay_selected(ctx: Mapping[str, Any], candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = {
        "all": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], np.ones(len(test_ids), dtype=bool), seed=43601, n=BOOTSTRAP_N),
        "t50": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], h == 50, seed=43602, n=BOOTSTRAP_N),
        "t100_raw_frame_diagnostic": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], h == 100, seed=43603, n=BOOTSTRAP_N),
        "hard_failure": di._bootstrap_ci_subset(test["selected_ade"], test["floor_ade"], hard_failure, seed=43604, n=BOOTSTRAP_N),
        "easy_degradation": di._bootstrap_ci_subset(test["floor_ade"], test["selected_ade"], easy, seed=43605, n=BOOTSTRAP_N),
    }
    near_bootstrap = {
        "final_near_005": ff._bootstrap_rate(final_near, seed=43621),
        "delta_final_minus_fc": ff._bootstrap_rate_delta(final_near, fc_near, seed=43622),
        "delta_final_minus_di": ff._bootstrap_rate_delta(final_near, di_near, seed=43623),
        "delta_final_minus_fb": ff._bootstrap_rate_delta(final_near, fb_near, seed=43624),
    }
    return {
        "val_ids": val_ids,
        "test_ids": test_ids,
...
def _gate(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    by_domain = replay["by_domain"]
    gates = {
        "source_fresh": payload["source"] == SOURCE,
        "fh_artifact_loaded": payload["fh_artifact"]["exists"] is True,
        ...
        "ucy_positive_safe": _positive_safe(by_domain.get("UCY", {})),
        "trajnet_positive_safe": _positive_safe(by_domain.get("TrajNet", {})),
    ...
    total = len(gates)
    verdict = "stage42_fi_fh_policy_freeze_replay_pass" if passed == total else "stage42_fi_fh_policy_freeze_replay_partial"
    return {"gates": gates, "passed": passed, "total": total, "verdict": verdict}


def _build_payload() -> dict[str, Any]:
    ensure_dir(OUT_DIR)
    fh_payload = read_json(fh.REPORT_JSON, {})
    if not fh_payload:
        fh_payload = fh.run_stage42_ucy_supported_fe_composer()
    ctx = _context()
    candidate = fh_payload["repair"]["selected"]["candidate"]
    replay = _replay_selected(ctx, candidate)
    test_metric = replay["test"]["metric"]
    test_diag = replay["test"]["diagnostics"]
    fh_metric = fh_payload["repair"]["test"]["metric_vs_floor"]
    fh_diag = fh_payload["repair"]["test"]["diagnostics"]
    metric_diffs = ff._metric_diffs(test_metric, fh_metric)
    diag_diffs = ff._metric_diffs(test_diag, fh_diag)
    policy_artifact = {
        "policy_name": "stage42_fh_ucy_supported_fe_composer",
        "source": SOURCE,
        "frozen_from": str(fh.REPORT_JSON),
        "frozen_at_utc": datetime.now(timezone.utc).isoformat(),
        "git_commit": ff._git_commit(),
        "selected_candidate": candidate,
        "policy_hash": ff._stable_hash(candidate),
        "runtime_inputs": [
        ...
        "validation_selection_rule": "UCY train-only internal validation plus existing validation; candidate selected on validation only",
        "test_usage_rule": "test labels used only for final evaluation and bootstrap, not for policy selection",
        "internal_val_group": ctx["internal_val_group"],
    ...
    payload: dict[str, Any] = {
        "source": SOURCE,
        "stage": "Stage42-FI FH policy freeze, 2000-bootstrap, and replay",
        ...
        "fh_artifact": {
            "exists": fh.REPORT_JSON.exists(),
            "report": str(fh.REPORT_MD),
            "verdict": fh_payload.get("stage42_fh_gate", {}).get("verdict"),
            "selected_candidate": candidate,
        },
        "frozen_policy": policy_artifact,
        "replay": {
            "candidate_exact_replay": candidate == fh_payload["repair"]["selected"]["candidate"],
            "test_rows": int(len(replay["test_ids"])),
            "test_metric_vs_floor": test_metric,
            "test_diagnostics": test_diag,
            "by_domain": replay["by_domain"],
            "metric_abs_diffs_vs_fh_artifact": metric_diffs,
            "diagnostic_abs_diffs_vs_fh_artifact": diag_diffs,
        ...
        "decision": "freeze_and_promote_stage42_fh_ucy_supported_fe_composer_if_replay_and_ci_hold",
        "notes": [
            "This step does not reselect thresholds or tune on test.",
            "It freezes the Stage42-FH selected candidate, recomputes the same policy, and adds 2000-bootstrap evidence.",
            "UCY internal validation is carved from train sources only; test rows remain final-eval only.",
            "Dataset-local/raw-frame boundaries remain unchanged.",
        ],
    }
    payload["stage42_fi_gate"] = _gate(payload)
    return _jsonable(payload)


def _render_report(payload: Mapping[str, Any]) -> list[str]:
    gate = payload["stage42_fi_gate"]
    ...
    lines = [
        ...
        f"- internal_val_group: `{payload['frozen_policy']['internal_val_group']}`",
        "",
    ...
    for domain, row in payload["replay"]["by_domain"].items():
        lines.append(
            ...
            f"{_pct(row['easy_degradation'])} | {_positive_safe(row)} |"
        )
    lines += [
        "",
        "## Exact Replay",
        ...
        f"- max metric abs diff vs FH artifact: `{payload['replay']['max_metric_abs_diff']}`",
        f"- max diagnostic abs diff vs FH artifact: `{payload['replay']['max_diagnostic_abs_diff']}`",
        "",
        "## Bootstrap CI",
        "",
...
def _render_policy(payload: Mapping[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"- internal_val_group: `{policy['internal_val_group']}`",
...
def _summary_section(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- gate: `{payload['stage42_fi_gate']['passed']} / {payload['stage42_fi_gate']['total']}`; verdict `{payload['stage42_fi_gate']['verdict']}`.",
            f"- frozen policy hash: `{payload['frozen_policy']['policy_hash']}`.",
            ...
            f"- dual-domain support: UCY `{_positive_safe(payload['replay']['by_domain'].get('UCY', {}))}`, TrajNet `{_positive_safe(payload['replay']['by_domain'].get('TrajNet', {}))}`.",
...
def _append_to_docs(payload: Mapping[str, Any]) -> None:
    ...
    for path in [README_RESULTS, M3W_README, WORK_SUMMARY, *PAPER_FILES]:
        ...
        path.write_text(_replace_text_section(old, "STAGE42_FI_FH_POLICY_FREEZE_REPLAY", section))


def _update_research_state(payload: Mapping[str, Any]) -> None:
    state = read_json(RESEARCH_STATE, {})
    state["current_stage"] = "Stage42-FI FH policy freeze / bootstrap / replay"
    state["current_verdict"] = payload["stage42_fi_gate"]["verdict"]
    state["stage42_fi_fh_policy_freeze_replay"] = {
        "source": payload["source"],
        "report": str(REPORT_MD),
        "policy": str(POLICY_JSON),
        "gate": str(GATE_MD),
        "verdict": payload["stage42_fi_gate"]["verdict"],
        "gates": f"{payload['stage42_fi_gate']['passed']}/{payload['stage42_fi_gate']['total']}",
        "policy_hash": payload["frozen_policy"]["policy_hash"],
        "test_metric_vs_floor": payload["replay"]["test_metric_vs_floor"],
        "by_domain": payload["replay"]["by_domain"],
        "bootstrap_ci": payload["bootstrap_ci"],
        "near_bootstrap_ci": payload["near_bootstrap_ci"],
        "claim_boundary": payload["claim_boundary"],
        "conclusion": "Stage42-FI freezes Stage42-FH and adds exact replay plus 2000-bootstrap evidence without reselection or test tuning.",
    }
    block = state.get("m3w_work_attempts_failures_successes_readme")
    if isinstance(block, dict):
        evidence = list(block.get("latest_included_evidence", []))
        if "Stage42-FI FH policy freeze / bootstrap / replay" not in evidence:
            evidence.append("Stage42-FI FH policy freeze / bootstrap / replay")
        block["latest_included_evidence"] = evidence
        block["source"] = "cached_verified_summary_from_stage18_to_stage42_reports_plus_stage42_es_to_fi_fresh_audits"
        block[
            "latest_conclusion"
        ] = "Stage42-FI freezes the Stage42-FH UCY-supported FE composer and adds exact replay plus 2000-bootstrap evidence."
        state["m3w_work_attempts_failures_successes_readme"] = block
    write_json(RESEARCH_STATE, state)


def run_stage42_fh_policy_freeze_replay() -> dict[str, Any]:
