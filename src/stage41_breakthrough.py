...
def eval_world_models() -> Dict[str, Any]:
    ...
    domain_focused = read_json("outputs/stage41_stratified_protocol/stage41_locked_v2_domain_focused.json", {})
    domain_composer = read_json("outputs/stage41_stratified_protocol/stage41_locked_v2_domain_expert_composer.json", {})
    ...
    fresh_self_gated = read_json("outputs/stage41_fresh_confirmation/stage41_fresh_self_gated_endpoint_candidate.json", {})
    ...
    if domain_composer:
        comparisons["Stage41_locked_v2_domain_expert_composer_candidate_not_deployable"] = domain_composer.get("best_metrics", {})
    if ensemble:
    ...
    if fresh_self_gated:
        comparisons["Stage41_fresh_self_gated_endpoint_candidate"] = fresh_self_gated.get("metrics_vs_floor", {})
        self_gated_metrics = dict(fresh_self_gated.get("metrics_vs_floor", {}) or {})
        self_gated_metrics["neural_endpoint_without_fallback"] = fresh_self_gated.get("self_gated_without_external_fallback_vs_source_rotation_base", {})
        self_gated_metrics["trajectory_endpoint_alignment_status"] = fresh_self_gated.get("trajectory_endpoint_alignment_status")
        if fresh_self_gated.get("protected_full_replacement_pass") and fresh_self_gated.get("no_external_fallback_safe_pass"):
            best_name = "fresh_self_gated_endpoint::binary_fde_neural_dynamics"
            best_metrics = self_gated_metrics
    positive_domains = 0
    ...
    result = {
        ...
        "locked_v2_domain_focused_available": bool(domain_focused),
        "locked_v2_domain_expert_composer_available": bool(domain_composer),
        ...
        "fresh_self_gated_endpoint_candidate_available": bool(fresh_self_gated),
        "protected_neural_candidate_note": "Fresh self-gated endpoint is promoted only as a binary-FDE-safe neural dynamics candidate. Continuous endpoint interpolation remains diagnostic until safety-floor endpoint geometry is repaired. It is not latent generative rollout and still keeps Stage5C and SMC disabled.",
    }
...
def failure_analysis() -> Dict[str, Any]:
    ...
    deployable_neural = bool(eval_report.get("neural_exceeds_stage37_by_gate_margin", False) and eval_report.get("positive_external_domains", 0) >= 2 and best.get("easy_degradation", 1.0) <= 0.02)
    result = {
        "source": "fresh_run",
        "answers": {
            ...
            "stage5c_ready": False,
            "smc_ready": False,
            "current_best_deployable": "Stage41 self-gated neural candidate" if deployable_neural else "Stage37 selector",
        },
        "failure_taxonomy": {
        ...
        "locked_v2_domain_focused": domain_focused.get("summary", {}),
        "locked_v2_domain_expert_composer": domain_composer.get("best_metrics", {}),
        ...
        "fresh_bounded_residual_candidate": fresh_bounded.get("metrics_vs_floor", {}),
        ...
        "fresh_self_gated_endpoint_candidate": fresh_self_gated.get("metrics_vs_floor", {}),
        "fresh_self_gated_without_external_fallback": fresh_self_gated.get("self_gated_without_external_fallback_vs_source_rotation_base", {}),
        "fresh_self_gated_endpoint_alignment": fresh_self_gated.get("trajectory_endpoint_alignment_status"),
...
def gates() -> Dict[str, Any]:
    ...
    fresh_self_gated = read_json("outputs/stage41_fresh_confirmation/stage41_fresh_self_gated_endpoint_candidate.json", {})
    self_gated_without = fresh_self_gated.get("self_gated_without_external_fallback_vs_source_rotation_base", {}) or {}
    endpoint_without = self_gated_without if fresh_self_gated.get("no_external_fallback_safe_pass") else best.get("neural_endpoint_without_fallback", {})
    ...
    rows = [
        ...
        ("Gate4l locked-v2 domain-focused hard experts run", bool(domain_focused), domain_focused.get("summary")),
        ("Gate4m locked-v2 domain expert composer run", bool(domain_composer), domain_composer.get("best_metrics")),
        ...
        ("Gate4w fresh self-gated endpoint candidate run", bool(fresh_self_gated), {"protected_full_replacement_pass": fresh_self_gated.get("protected_full_replacement_pass"), "no_external_fallback_safe_pass": fresh_self_gated.get("no_external_fallback_safe_pass"), "metrics_vs_floor": fresh_self_gated.get("metrics_vs_floor"), "self_gated_without_external_fallback": fresh_self_gated.get("self_gated_without_external_fallback_vs_source_rotation_base"), "endpoint_alignment": fresh_self_gated.get("trajectory_endpoint_alignment_status")}),
        ...
        ("Gate10 neural without fallback not catastrophic", endpoint_not_catastrophic, {"evidence": endpoint_without, "source": "fresh_self_gated_endpoint" if fresh_self_gated.get("no_external_fallback_safe_pass") else "best_raw_neural_endpoint", "endpoint_alignment": fresh_self_gated.get("trajectory_endpoint_alignment_status")}),
...
def write_final_reports(gate_result: Mapping[str, Any], eval_report: Mapping[str, Any]) -> None:
    ...
    deployed = str(gate_result.get("current_verdict", "")).startswith("stage41_self_gated_neural_candidate")
    best = eval_report.get("best_stage41_metrics", {})
    all_agent = read_json(OUT_DIR / "stage41_all_agent_eval.json", {})
    calibrator = read_json(OUT_DIR / "stage41_intervention_calibrator_eval.json", {})
    t50_rescue = read_json(OUT_DIR / "stage41_t50_rescue.json", {})
    ...
    fresh_bounded_floor = fresh_bounded.get("metrics_vs_floor", {}) or {}
    fresh_bounded_without = fresh_bounded.get("bounded_without_fallback_vs_source_rotation_base", {}) or {}
    ...
    lines = [
        ...
        "",
        f"- available: `{bool(domain_focused)}`",
        f"- deployment decision: `{domain_focused.get('deployment_decision')}`",
        f"- summary: `{domain_focused.get('summary')}`",
        "",
        "## Locked-v2 Domain Expert Composer",
        "",
        f"- available: `{bool(domain_composer)}`",
        f"- deployment decision: `{domain_composer.get('deployment_decision')}`",
        f"- Stage37-margin result: `{domain_composer.get('neural_exceeds_stage37_by_gate_margin')}`",
        f"- best metrics: `{domain_composer.get('best_metrics')}`",
        ...
        "",
        f"- available: `{bool(relaxed_easy)}`",
        f"- best ensemble: `{relaxed_easy.get('best_ensemble')}`",
        f"- deployment decision: `{relaxed_easy.get('deployment_decision')}`",
        f"- Stage37-margin result: `{relaxed_easy.get('neural_exceeds_stage37_by_gate_margin')}`",
        f"- best metrics: `{relaxed_easy.get('best_metrics')}`",
        ...
        "",
        f"- available: `{bool(domain_safe_relaxed)}`",
        f"- best candidate: `{domain_safe_relaxed.get('best_candidate')}`",
        f"- deployment decision: `{domain_safe_relaxed.get('deployment_decision')}`",
        f"- Stage37-margin result: `{domain_safe_relaxed.get('neural_exceeds_stage37_by_gate_margin')}`",
        f"- max per-domain easy degradation: `{domain_safe_relaxed.get('max_domain_easy_degradation')}`",
        ...
        "",
        f"- available: `{bool(source_rotation_confirmation)}`",
        f"- deployment decision: `{source_rotation_confirmation.get('deployment_decision')}`",
        f"- fresh confirmation pass: `{source_rotation_confirmation.get('fresh_confirmation_pass')}`",
        f"- full replacement pass: `{source_rotation_confirmation.get('full_replacement_pass')}`",
        f"- all: `{source_rotation_metrics.get('all_improvement')}`",
        f"- t50: `{source_rotation_metrics.get('t50_improvement')}`",
        f"- t100 diagnostic: `{source_rotation_metrics.get('t100_improvement')}`",
        f"- hard/failure: `{source_rotation_metrics.get('hard_failure_improvement')}`",
        f"- easy degradation: `{source_rotation_metrics.get('easy_degradation')}`",
        f"- t50 oracle ceiling: `{source_rotation_confirmation.get('t50_oracle_ceiling')}`",
        ...
        "",
        f"- available: `{bool(fresh_bounded)}`",
        f"- deployment decision: `{fresh_bounded.get('deployment_decision')}`",
        f"- protected full replacement pass: `{fresh_bounded.get('protected_full_replacement_pass')}`",
        f"- no-fallback safe pass: `{fresh_bounded.get('no_fallback_safe_pass')}`",
        f"- vs floor all: `{fresh_bounded_floor.get('all_improvement')}`",
        f"- vs floor t50: `{fresh_bounded_floor.get('t50_improvement')}`",
        f"- vs floor t100 diagnostic: `{fresh_bounded_floor.get('t100_improvement')}`",
        f"- vs floor hard/failure: `{fresh_bounded_floor.get('hard_failure_improvement')}`",
        f"- bounded without fallback easy degradation: `{fresh_bounded_without.get('easy_degradation')}`",
        ...
        "",
        f"- available: `{bool(fresh_interpolation)}`",
        f"- deployment decision: `{fresh_interpolation.get('deployment_decision')}`",
        f"- protected full replacement pass: `{fresh_interpolation.get('protected_full_replacement_pass')}`",
        f"- no-fallback safe pass: `{fresh_interpolation.get('no_fallback_safe_pass')}`",
        f"- best alpha: `{fresh_interpolation.get('best_alpha')}`",
        f"- vs floor all: `{fresh_interpolation_floor.get('all_improvement')}`",
        f"- vs floor t50: `{fresh_interpolation_floor.get('t50_improvement')}`",
        f"- vs floor t100 diagnostic: `{fresh_interpolation_floor.get('t100_improvement')}`",
        f"- vs floor hard/failure: `{fresh_interpolation_floor.get('hard_failure_improvement')}`",
        f"- vs floor easy: `{fresh_interpolation_floor.get('easy_degradation')}`",
        f"- vs source-rotation base all: `{fresh_interpolation_base.get('all_improvement')}`",
        f"- vs source-rotation base t50: `{fresh_interpolation_base.get('t50_improvement')}`",
        ...
        "",
        f"- available: `{bool(fresh_self_gated)}`",
        f"- deployment decision: `{fresh_self_gated.get('deployment_decision')}`",
        f"- protected full replacement pass: `{fresh_self_gated.get('protected_full_replacement_pass')}`",
        f"- no external fallback safe pass: `{fresh_self_gated.get('no_external_fallback_safe_pass')}`",
        f"- vs floor all: `{fresh_self_gated_floor.get('all_improvement')}`",
        f"- vs floor t50: `{fresh_self_gated_floor.get('t50_improvement')}`",
        f"- vs floor t100 diagnostic: `{fresh_self_gated_floor.get('t100_improvement')}`",
        f"- vs floor hard/failure: `{fresh_self_gated_floor.get('hard_failure_improvement')}`",
        f"- vs floor easy: `{fresh_self_gated_floor.get('easy_degradation')}`",
        f"- self-gated vs source-rotation base all: `{fresh_self_gated_base.get('all_improvement')}`",
        f"- self-gated vs source-rotation base t50: `{fresh_self_gated_base.get('t50_improvement')}`",
        f"- self-gated vs source-rotation base t100: `{fresh_self_gated_base.get('t100_improvement')}`",
        f"- self-gated vs source-rotation base hard: `{fresh_self_gated_base.get('hard_failure_improvement')}`",
        f"- self-gated easy degradation: `{fresh_self_gated_base.get('easy_degradation')}`",
...
- locked-v2 domain-focused experts: deployment `{domain_focused.get('deployment_decision')}`, summary `{domain_focused.get('summary')}`.
- locked-v2 domain expert composer: deployment `{domain_composer.get('deployment_decision')}`, margin result `{domain_composer.get('neural_exceeds_stage37_by_gate_margin')}`, hard `{(domain_composer.get('best_metrics') or {}).get('hard_failure_improvement')}`.
...
"""
    ...
    reports.add("outputs/stage41_stratified_protocol/stage41_locked_v2_domain_focused.md")
    reports.add("outputs/stage41_stratified_protocol/stage41_locked_v2_domain_expert_composer.md")
    ...
    if domain_focused:
        stage41_state["locked_v2_domain_focused_candidate"] = {
            "source": domain_focused.get("source"),
            "protocol_status": domain_focused.get("protocol_status"),
            "deployment_decision": domain_focused.get("deployment_decision"),
            "summary": domain_focused.get("summary"),
            "conclusion": domain_focused.get("caveat"),
        }
    if domain_composer:
        stage41_state["locked_v2_domain_expert_composer"] = {
            "source": domain_composer.get("source"),
            "protocol_status": domain_composer.get("protocol_status"),
            "deployment_decision": domain_composer.get("deployment_decision"),
            "best_metrics": domain_composer.get("best_metrics"),
            "choices": domain_composer.get("choices"),
            "conclusion": domain_composer.get("caveat"),
        }
    if ensemble:
    ...
    if relaxed_easy:
        stage41_state["locked_v2_relaxed_easy_budget_candidate"] = {
            "source": relaxed_easy.get("source"),
            "protocol_status": relaxed_easy.get("protocol_status"),
            "deployment_decision": relaxed_easy.get("deployment_decision"),
            "best_name": relaxed_easy.get("best_ensemble"),
            "best_mode": relaxed_easy.get("best_mode"),
            "best_metrics": relaxed_easy.get("best_metrics"),
            "conclusion": relaxed_easy.get("caveat"),
        }
    if domain_safe_relaxed:
        stage41_state["locked_v2_domain_safe_relaxed_candidate"] = {
            "source": domain_safe_relaxed.get("source"),
            "protocol_status": domain_safe_relaxed.get("protocol_status"),
            "deployment_decision": domain_safe_relaxed.get("deployment_decision"),
            "best_name": domain_safe_relaxed.get("best_candidate"),
            "best_metrics": domain_safe_relaxed.get("best_metrics"),
    ...
    if source_rotation_confirmation:
        stage41_state["source_rotation_fresh_confirmation"] = {
            "source": source_rotation_confirmation.get("source"),
            "protocol_status": source_rotation_confirmation.get("protocol_status"),
            "deployment_decision": source_rotation_confirmation.get("deployment_decision"),
            "fresh_confirmation_pass": source_rotation_confirmation.get("fresh_confirmation_pass"),
            "full_replacement_pass": source_rotation_confirmation.get("full_replacement_pass"),
            "stage37_any_core_margin_pass": source_rotation_confirmation.get("stage37_any_core_margin_pass"),
            "stage37_all_core_margin_pass": source_rotation_confirmation.get("stage37_all_core_margin_pass"),
            "best_name": source_rotation_confirmation.get("best_name"),
            "best_metrics": source_rotation_metrics,
            "positive_external_domains": source_rotation_confirmation.get("positive_external_domains"),
            "max_domain_easy_degradation": source_rotation_confirmation.get("max_domain_easy_degradation"),
            "t50_oracle_ceiling": source_rotation_confirmation.get("t50_oracle_ceiling"),
    ...
    if fresh_bounded:
        stage41_state["fresh_bounded_residual_candidate"] = {
            "source": fresh_bounded.get("source"),
            "protocol_status": fresh_bounded.get("protocol_status"),
            "deployment_decision": fresh_bounded.get("deployment_decision"),
            "protected_full_replacement_pass": fresh_bounded.get("protected_full_replacement_pass"),
            "no_fallback_safe_pass": fresh_bounded.get("no_fallback_safe_pass"),
            "metrics_vs_floor": fresh_bounded_floor,
            "bounded_without_fallback_vs_source_rotation_base": fresh_bounded_without,
            "conclusion": fresh_bounded.get("caveat"),
        }
    if fresh_interpolation:
        stage41_state["fresh_endpoint_interpolation_candidate"] = {
            "source": fresh_interpolation.get("source"),
            "protocol_status": fresh_interpolation.get("protocol_status"),
            "deployment_decision": fresh_interpolation.get("deployment_decision"),
            "best_alpha": fresh_interpolation.get("best_alpha"),
            "protected_full_replacement_pass": fresh_interpolation.get("protected_full_replacement_pass"),
            "no_fallback_safe_pass": fresh_interpolation.get("no_fallback_safe_pass"),
            "metrics_vs_floor": fresh_interpolation_floor,
            "metrics_vs_source_rotation_base": fresh_interpolation_base,
    ...
    if fresh_self_gated:
        stage41_state["fresh_self_gated_endpoint_candidate"] = {
            "source": fresh_self_gated.get("source"),
            "protocol_status": fresh_self_gated.get("protocol_status"),
            "deployment_decision": fresh_self_gated.get("deployment_decision"),
            "protected_full_replacement_pass": fresh_self_gated.get("protected_full_replacement_pass"),
            "no_external_fallback_safe_pass": fresh_self_gated.get("no_external_fallback_safe_pass"),
            "metrics_vs_floor": fresh_self_gated_floor,
            "self_gated_without_external_fallback_vs_source_rotation_base": fresh_self_gated_base,
            "raw_ungated_endpoint_vs_source_rotation_base": fresh_self_gated_raw,
