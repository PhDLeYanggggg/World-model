"""Fixed readout against frozen-region fitting, including per-scene failures."""
from pathlib import Path
import numpy as np
from scripts import run_m3w_bounded_cost as bounded
from scripts.run_m3w_adaptive_region_cost import weighted_data, check_state
...
def evaluate(cfg,data,views,predictions,prior,refs,identity,records,states,beat,verify=False):
    ...
    result=dict(identity=identity,result_source='fresh_12_adaptive_fit_region_heads_and_readout',
        ...
        refresh_records=[dict(view=key,step=r['step'],selected=int(r['selected'].sum()),
            complete_selected=r['complete_selected'],mean_weight=r['mean_weight'])
            for key,cp in states.items() for r in cp['refreshes']],
        refresh_seconds=sum(r['fit']['refresh_seconds'] for r in records.values()),
