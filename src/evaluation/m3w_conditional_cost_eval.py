"""Readout for the fixed fit-region weighted cost-head experiment."""
from pathlib import Path
import numpy as np
from scripts import run_m3w_bounded_cost as bounded
from scripts.run_m3w_conditional_cost import weighted_data, check_state
...
def gate(summary,contrast):
    return dict(positive_primary_ci=contrast['ci95_pp'][0]>0,
...
def cost_means(old,new,y,use):
    if not use.any():return None
    return dict(realized_harm=float(y[use,1].mean()),realized_benefit=float(y[use,0].mean()),
        old_predicted_harm=float(old[use,1].mean()),new_predicted_harm=float(new[use,1].mean()),
        old_predicted_benefit=float(old[use,0].mean()),new_predicted_benefit=float(new[use,0].mean()),
        old_MSE=float(((old[use]-y[use])**2).mean()),new_MSE=float(((new[use]-y[use])**2).mean()))
...
def evaluate(cfg,data,views,predictions,prior,refs,identity,records,states,beat,verify=False):
    ...
    for key,meta in views.items():
        ...
        with np.load(ROOT/old[key]['path'],allow_pickle=False) as z:
            ...
            anchor=z['anchor'].copy();old_scores[key]=z['tempered_score'].copy()
            old_bits[key]=z['tempered_strict_stop'].copy()
    ...
    contrasts={p:{a:paired_scene_contrast(
        ...
        for a in ('tempered','native','fraction')} for p in POLICIES}
    checks=gate(summaries['strict_stop'],contrasts['strict_stop']['tempered'])
    result=dict(identity=identity,result_source='fresh_12_fit_region_weighted_heads_and_readout',
        reference_source='cached_verified_equal_budget_objective_controls',summaries=summaries,
