"""Past-context proximity for explicit fixed choices, without future-label inputs."""
import json
...
def audit(cfg, data, proposals, choices, policy_arms, beat):
    ...
    for seed in cfg['seeds']:
        if set(choices[seed]) != set(policy_arms): raise ValueError('Explicit policy/arm mapping required')
        ...
        rows = {s: dict(queries=0, known_edges=0, unknown_edges=0, unsupported_context_rows=0,
                       policy_excess_sum={p: 0. for p in policy_arms}) for s in cfg['sites']}
        for record in context['records']:
            ...
            for frame in np.unique(c['context_frame_ids']):
                ...
                target = c['context_target_rows'][ix] >= 0; tid = c['context_target_rows'][ix][target]
                if not len(tid): raise ValueError('Expected past-eligible query targets')
                ...
                tables = {}
                for arm in sorted(set(policy_arms.values())):
                    q = b.copy(); q[target] = native[arm][tid]
                    tables[arm] = proximity_cost_table(b, q, safe_edges, distance_threshold=.1*radius)
                for name, arm in policy_arms.items():
                    chosen = np.zeros(len(ix), bool); chosen[target] = choices[seed][name][tid]
                    cost = tables[arm][np.arange(len(safe_edges)), chosen[safe_edges[:, 0]].astype(int),
                                       chosen[safe_edges[:, 1]].astype(int)]
