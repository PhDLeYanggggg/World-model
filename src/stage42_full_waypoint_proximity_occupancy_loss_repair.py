...
REPORT_JSON = OUT_DIR / "full_waypoint_proximity_occupancy_loss_repair_stage42.json"
REPORT_MD = OUT_DIR / "full_waypoint_proximity_occupancy_loss_repair_stage42.md"
REPORT_CSV = OUT_DIR / "full_waypoint_proximity_occupancy_loss_repair_stage42.csv"
GATE_MD = OUT_DIR / "stage42_stage_dh_gate.md"

AM_JSON = OUT_DIR / "source_level_full_waypoint_eval_stage42.json"
DG_JSON = OUT_DIR / "full_waypoint_all_hard_loss_repair_stage42.json"
...
def _graph_signals(graph: np.ndarray) -> dict[str, np.ndarray]:
    min_dist = graph[:, 1].astype(np.float64)
    density_r1 = graph[:, 4].astype(np.float64)
    density_r2 = graph[:, 5].astype(np.float64)
    density_r5 = graph[:, 6].astype(np.float64)
    inv_min = graph[:, 3].astype(np.float64)
    mean_closing = np.zeros(len(graph), dtype=np.float64)
    for k in range(graph_ctx.K_NEIGHBORS):
        mean_closing += np.maximum(graph[:, 13 + 6 * k].astype(np.float64), 0.0)
    mean_closing /= float(graph_ctx.K_NEIGHBORS)
    close = np.isfinite(min_dist) & (min_dist < 1.0)
    very_close = np.isfinite(min_dist) & (min_dist < 0.5)
    return {
        "min_dist": min_dist,
        "density_r1": density_r1,
        "density_r2": density_r2,
        "density_r5": density_r5,
        "inv_min": inv_min,
        "mean_closing": mean_closing,
        "close": close.astype(np.float64),
        "very_close": very_close.astype(np.float64),
    }


def _loss_weights(data: Mapping[str, np.ndarray], graph: np.ndarray, train_mask: np.ndarray, variant: str) -> np.ndarray:
    h = data["horizon"].astype(int)
    hard_failure = data["hard"].astype(bool) | data["failure"].astype(bool)
    easy = data["easy"].astype(bool)
    sig = _graph_signals(graph)
    w = np.ones(len(h), dtype=np.float64)
    if variant == "proximity_close_weighted":
        w += 2.5 * sig["close"] + 4.0 * sig["very_close"] + 1.0 * sig["mean_closing"]
    elif variant == "occupancy_proxy_density_weighted":
        w += 2.0 * sig["density_r1"] + 1.2 * sig["density_r2"] + 0.5 * sig["density_r5"]
    elif variant == "proximity_hard_long_weighted":
        w += 2.0 * sig["close"] + 3.0 * hard_failure.astype(np.float64) + 2.0 * (h == 50) + 2.8 * (h == 100)
    elif variant == "physical_easy_safe_weighted":
        w += 3.0 * sig["very_close"] + 1.5 * sig["inv_min"] + 1.0 * hard_failure.astype(np.float64)
        w[easy] *= 0.75
    else:
        raise ValueError(f"Unknown Stage42-DH variant: {variant}")
    ...
    return [
        {
            ...
            "weights": _loss_weights(data, graph, train_mask, "proximity_close_weighted"),
        },
        {
            "variant": "occupancy_proxy_density_weighted",
            "features": base_features,
            "feature_mode": "stage42_am_features",
            "weights": _loss_weights(data, graph, train_mask, "occupancy_proxy_density_weighted"),
        },
        {
            "variant": "proximity_hard_long_weighted",
            "features": base_features,
            "feature_mode": "stage42_am_features",
            "weights": _loss_weights(data, graph, train_mask, "proximity_hard_long_weighted"),
        },
        {
            "variant": "physical_easy_safe_weighted",
            "features": base_features,
            "feature_mode": "stage42_am_features",
            "weights": _loss_weights(data, graph, train_mask, "physical_easy_safe_weighted"),
        },
        {
            "variant": "graph_augmented_proximity_hard_long",
...
def _evaluate_candidates(
    ...
    best_score = -1e9
    for spec in _candidate_specs(data, base_features, graph, train_mask):
        ...
        for lam in LAMBDAS:
            ...
            score = dg._selection_score(val_metric)
            row = {
    ...
    if best is None:
        raise RuntimeError("No Stage42-DH proximity/occupancy loss candidate evaluated.")
    h = data["horizon"].astype(int)
    hard_failure = data["hard"].astype(bool) | data["failure"].astype(bool)
    easy = data["easy"].astype(bool)
    ...
    best_public = {
        ...
        if k not in {"coef", "selected_ade", "selected_fde", "switch", "pred_ade", "pred_fde", "floor_ade", "floor_fde"}
    }
    return {
        ...
        "metrics": {
            ...
            "ungated_selected_candidate": am._metric(best["pred_ade"], best["floor_ade"], data, np.ones(len(h), dtype=bool), test_mask),
            "protected_selected_candidate": am._metric(best["selected_ade"], best["floor_ade"], data, best["switch"], test_mask),
            "protected_selected_candidate_fde": am._metric(best["selected_fde"], best["floor_fde"], data, best["switch"], test_mask),
        },
        "bootstrap": {
            "all": am._bootstrap_ci(best["selected_ade"], best["floor_ade"], test_mask, seed=42071),
            "t50": am._bootstrap_ci(best["selected_ade"], best["floor_ade"], test_mask & (h == 50), seed=42072),
            "t100_raw_frame_diagnostic": am._bootstrap_ci(best["selected_ade"], best["floor_ade"], test_mask & (h == 100), seed=42073),
            "hard_failure": am._bootstrap_ci(best["selected_ade"], best["floor_ade"], test_mask & hard_failure, seed=42074),
            "easy_degradation": am._bootstrap_ci(best["floor_ade"], best["selected_ade"], test_mask & easy, seed=42075),
        },
        "by_domain": {
...
def _compare_to_stage42_am(eval_result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    current = eval_result["metrics"]["protected_selected_candidate"]
    return {
...
def _build_payload() -> dict[str, Any]:
    ...
    eval_result = _evaluate_candidates(data, split, labels, floor, features, graph)
    comparison = _compare_to_stage42_am(eval_result)
    metric = eval_result["metrics"]["protected_selected_candidate"]
    ...
    result: dict[str, Any] = {
        ...
        "stage": "Stage42-DH full-waypoint proximity/occupancy-proxy weighted loss repair",
        ...
        "feature_schema": {
            "source": "fresh_stage42_dh_graph_proximity_weight_schema",
            ...
            "future_inputs": False,
            "variants": [
                "proximity_close_weighted",
                "occupancy_proxy_density_weighted",
                "proximity_hard_long_weighted",
                "physical_easy_safe_weighted",
                "graph_augmented_proximity_hard_long",
            ],
            "lambdas": LAMBDAS,
        },
        ...
        "deployment_decision": {
            "promote_proximity_occupancy_loss_candidate": bool(improves_am),
            "decision": "promote_stage42_dh_proximity_occupancy_loss_candidate"
            if improves_am
            else "proximity_occupancy_loss_not_enough_keep_stage42_am_or_cq_floor",
    ...
    result["stage42_dh_gate"] = _gate(result)
    return result


def _gate(result: Mapping[str, Any]) -> dict[str, Any]:
    m = result["model"]["metrics"]["protected_selected_candidate"]
    ...
    gates = {
        ...
        "graph_proximity_features_built": result["graph_stats"]["rows_with_neighbors"] > 0 and result["feature_schema"]["graph_feature_count"] > 0,
        "proximity_occupancy_variants_run": result["model"]["candidate_count"] >= len(result["feature_schema"]["variants"]),
        ...
        "no_future_or_test_leakage": all(
            [
                ...
                no_leak["graph_features_current_and_past_only"] is True,
    ...
    total = len(gates)
    if passed == total:
        verdict = "stage42_dh_proximity_occupancy_loss_repair_pass_promotable"
        ...
        verdict = "stage42_dh_proximity_occupancy_loss_repair_partial"
            ...
            writer.writerow(
                {
                    ...
                    "mean_train_weight": row["mean_train_weight"],
                    "max_train_weight": row["max_train_weight"],
...
def _render_report(result: Mapping[str, Any]) -> list[str]:
    gate = result["stage42_dh_gate"]
    selected = result["model"]["selected"]
    metric = result["model"]["metrics"]["protected_selected_candidate"]
    ungated = result["model"]["metrics"]["ungated_selected_candidate"]
    delta = result["comparison_to_stage42_am"]["delta_vs_stage42_am"]
    lines = [
        ...
        *[f"- {fact}" for fact in CURRENT_FACTS],
        ...
        "",
        f"- graph_stats: `{result['graph_stats']}`",
        "",
        "## Test Once vs Train-Horizon Causal Floor",
        "",
        "| candidate | all | t50 | t100 raw diag | hard/failure | easy degradation | switch |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: |",
        f"| ungated selected candidate | {_pct(ungated['all_improvement'])} | {_pct(ungated['t50_improvement'])} | {_pct(ungated['t100_raw_frame_diagnostic_improvement'])} | {_pct(ungated['hard_failure_improvement'])} | {_pct(ungated['easy_degradation'])} | {_pct(ungated['switch_rate'])} |",
        f"| protected selected candidate | {_pct(metric['all_improvement'])} | {_pct(metric['t50_improvement'])} | {_pct(metric['t100_raw_frame_diagnostic_improvement'])} | {_pct(metric['hard_failure_improvement'])} | {_pct(metric['easy_degradation'])} | {_pct(metric['switch_rate'])} |",
        "",
...
def _refresh_lines(result: Mapping[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"- selected candidate: `{selected['variant']}` with `{selected['feature_mode']}` and lambda `{selected['lambda']}`.",
...
def _refresh_readmes(result: Mapping[str, Any]) -> None:
    ...
    for path in [README_RESULTS, M3W_README, GOAL_README, CURRENT_RETROSPECTIVE]:
        _replace_section(path, "STAGE42_DH_FULL_WAYPOINT_PROXIMITY_OCCUPANCY_LOSS_REPAIR", lines)


def _refresh_research_state(result: Mapping[str, Any]) -> None:
    state = read_json(RESEARCH_STATE, {}) if RESEARCH_STATE.exists() else {}
    state["current_stage"] = "Stage42-DH full-waypoint proximity/occupancy-proxy loss repair"
    state["current_verdict"] = result["stage42_dh_gate"]["verdict"]
    state["stage42_dh_full_waypoint_proximity_occupancy_loss_repair"] = {
        "source": result["source"],
        "report": str(REPORT_MD),
        "gate": str(GATE_MD),
        "verdict": result["stage42_dh_gate"]["verdict"],
        "gates": f"{result['stage42_dh_gate']['passed']}/{result['stage42_dh_gate']['total']}",
        "deployment_decision": result["deployment_decision"],
        "selected": {
            "variant": result["model"]["selected"]["variant"],
            "feature_mode": result["model"]["selected"]["feature_mode"],
            "lambda": result["model"]["selected"]["lambda"],
            "val_score": result["model"]["selected"]["val_score"],
        },
        "test_metric_vs_floor": result["model"]["metrics"]["protected_selected_candidate"],
...
def run_stage42_full_waypoint_proximity_occupancy_loss_repair(*, refresh_readmes: bool = True) -> dict[str, Any]:
...
if __name__ == "__main__":
    run_stage42_full_waypoint_proximity_occupancy_loss_repair()
