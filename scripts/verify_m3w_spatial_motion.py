...
def main():
    output = ROOT/'data/stage_cvpr2027_experiments/spatial_motion'
    reports = ROOT/'outputs/publication_readiness_2026_09/spatial_motion'
    report = json.loads((reports/'report.json').read_text())
    replay = json.loads((reports/'replay.json').read_text())
    if len(report['trials']) != 36 or len(replay['trials']) != 36 or not all(t['prediction_exact'] for t in replay['trials']):
    ...
    if not new['lowpass_replay_exact']:
        raise ValueError('Native downsample/source replay failed')
    checkpoints = sorted((output/'checkpoints').glob('*.pt'))
    if len(checkpoints) != 36:
        raise ValueError('Require exactly 36 final checkpoints')
    checkpoint_info = []
    ...
    with (output/'completed_resume.log').open('w') as log:
        finished = subprocess.run([sys.executable, 'scripts/run_m3w_spatial_motion.py', '--registration',
                                  'configs/m3w_spatial_motion.json'], cwd=ROOT, stdout=log, stderr=log)
    ...
    result = dict(result_source='cached_verified_actual_checkpoint_replay_and_resume',
        ...
        replay_exact_models=36, native_downsample_to_registered_source_exact=True,
        ...
        synthetic_and_focused_tests='17 passed; full legacy suite not rerun')
