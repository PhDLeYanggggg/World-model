"""Predeclared auxiliary-cost contrasts; no policy or best-role selection."""
import json
...
COMPARISONS={'aux_vs_control':('membership_aux','cost_only'),
    'aux_vs_original':('membership_aux','original_mean'),
    'aux_vs_conditional':('membership_aux','conditional'),'control_vs_original':('cost_only','original_mean')}
...
def gates_for(summary):
    ...
    return dict(primary_six_positive_vs_both=primary,tail_coverage_all_harm_guards=guards,
...
def aggregates(rows,cfg):
    ...
    pop={p:{s:{a:summarize_population([f['metrics'][a][s] for r in rows if r['pair']==p for f in r['folds']])
        for a in ('original_mean','cost_only','membership_aux','conditional')} for s in ('all','envelope_positive')} for p in cfg['pairs']}
    transport={}
    for key,(new,old) in COMPARISONS.items():
        transport[key]={}
    ...
    return dict(contrasts=cs,summary=summary,populations=pop,fit_transport=transport,
        control_max_abs_error=max(f['control_prediction_max_abs_error'] for r in rows for f in r['folds']),gates=gates_for(summary))
...
def main():
    ...
    fits=[dict(group=r['input']['group']['group'],pair=r['input']['pair'],held=r['input']['held'],arm=r['input']['arm'],fit=r['fit']) for r in receipts]
    ...
    run.immutable_json(run.PUBLIC/'compute_receipt.json',dict(heads=len(fits),updates=sum(r['fit']['step'] for r in fits),
        ...
        parameters_per_arm=24901,threads=4,interop=1,workers=0,real_torch=True,remote_jobs_submitted=0,
        extra_pretraining=False,elapsed_excludes_loading_inference_and_verification=True))
    lines=['# Auxiliary Membership Cost Results','','## Material Passport',
        '288 fresh Torch fits / 576,000 updates. Source forecasts and original costs are cached_verified.',
        'All six source assignments, three seeds and four held localities; source development only.',
        'Membership is auxiliary supervision, not a multiplicative cost factor or an inference label.','',
        '| Comparison / pair / roles | Easy-harm MSE gain % | 95% locality CI |','|---|---:|---:|']
    ...
    lines+=['','Three seeds averaged within locality, then 3,000 resamples of four localities. Roles and overlapping windows are dependent, exploratory and not multiplicity-adjusted.',
        'Each new arm has 24,901 parameters. Total objectives differ by auxiliary BCE; cost loss is reported separately.',
        'Cost control maximum absolute prediction difference from original: '+str(a['control_max_abs_error']),
        'Primary requires all six full-input cost intervals positive against both control and original, with tail/coverage/all-harm guards.',
        'No metric/seconds, human-gold, physical-safety, true3D, foundation, independent-confirmation or deployment claim. Stage5C/SMC off.','',
