"""Publish every registered ranking-auxiliary result, training trace and check."""
import argparse
import csv
import json
...
from scripts import run_m3w_european_ranked_hurdle as run
...
def name(value):
    return value.replace('product', 'control').replace('hurdle', 'ranked')
...
def reports(a):
    pub = run.PUBLIC
    rows = ['# Ranking-Augmented Hurdle Heads: Every Registered View', '',
        'Control = frozen occurrence/severity hurdle; ranked = the same model plus ranking loss.',
        'No product-MSE head was fitted in this experiment. All count controls are offline diagnostics.', '',
    ...
    versus = {k: {s: compact_metric(m) for s, m in r.items()} for k, r in a['neural_vs_damping'].items()}
    vstats = {}
    for arm in ('control_original', 'ranked_original'):
        ...
        for subset in ('all', 'easy', 'hard'):
            ...
            assert len(ms) == 18
            vstats[arm][subset] = dict(comparisons=len(ms),
...
def complete(a):
    ...
    for pid, v in last.items():
        ...
        if v['state'] != 'phase_complete' or 'run_m3w_european_ranked_hurdle.py' in process.stdout:
            raise ValueError('Training/evaluation phase is not terminal')
    parent = json.loads((run.coverage.PUBLIC/'completion_checks.json').read_text())
    tests = parent['scoped_test_files'] + ['tests/test_m3w_ranked_hurdle.py']
    ...
    if result.returncode or not re.search(r'\b234 passed\b', result.stdout):
        raise ValueError('Scoped tests failed')
    run.json_write(run.PUBLIC/'completion_checks.json', dict(result_source='fresh_run_ranking_training_and_verification',
        utc=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), analysis_sha256=run.digest(run.PUBLIC/'analysis.json'),
        reporter_sha256=run.digest(Path(__file__)), training_metrics_sha256=run.digest(run.PUBLIC/'training_metrics.json'),
        tests_sha256=run.digest(run.PUBLIC/'scoped_tests.txt'), scoped_test_files=tests, tests_passed=234,
        full_legacy_suite_run=False, heads=36, updates=72000, views=216, old_controls=36,
        completed_pids=sorted(last), all_required_phases_finished=True, reserved_roles_opened=False,
        deployment_changed=False, stage5c_executed=False, smc_enabled=False, submission_ready=False))
    print(json.dumps(dict(heads=36, updates=72000, views=216, tests=234)))
...
def figures(a):
    ...
    for ax, component, title in zip(axes, ('ranking_at_product_count', 'ranking_at_hurdle_count'),
            ('At frozen control common counts', 'At rank-augmented common counts')):
        ...
        ax.set_xlabel('Ranking-auxiliary advantage (pp of CV-normalized ADE)')
    ...
    fig.suptitle('Does direct ranking supervision improve ordering at fixed coverage?\n'
    ...
    for key, row in training.items():
        trace = row['fit']['trace']; color = colors[0 if key.startswith('neural') else 1]
        for ax, metric in zip(axes, ('loss', 'ranking_loss')):
            ax.plot([v['step'] for v in trace], [v[metric] for v in trace], color=color, alpha=.45, linewidth=.8)
            ax.set_xlabel('Optimizer step'); ax.set_ylabel(metric); ax.grid(alpha=.2)
    fig.suptitle('All 36 fitting traces; total and weighted-pair logistic losses, no validation selection')
    for ext in ('svg', 'png'):
...
def main():
    ...
    p.add_argument('--complete', action='store_true'); p.add_argument('--figures', action='store_true')
    args = p.parse_args(); a = verify(); reports(a)
    if args.complete:
        complete(a)
    if args.figures:
        figures(a)
