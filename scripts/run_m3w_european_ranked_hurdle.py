"""Train one matched ranking auxiliary; freeze all arms before development readout."""
import argparse
import fcntl
import json
import os
...
from src.world_model.m3w_ranked_hurdle import fit
...
PUBLIC = ROOT/'outputs/publication_readiness_2026_09/european_ranked_hurdle_v1'
PRIVATE = ROOT/'data/stage_cvpr2027_experiments/european_ranked_hurdle_v1'
CONFIG = 'configs/m3w_european_ranked_hurdle_v1.json'
FILES = (CONFIG, 'scripts/run_m3w_european_ranked_hurdle.py', 'src/world_model/m3w_ranked_hurdle.py',
         'tests/test_m3w_ranked_hurdle.py',
         'outputs/publication_readiness_2026_09/european_ranked_hurdle_v1/registration.md')
...
def train(ctx, *, pilot, resume):
    ...
    for name, candidate, fold, seed, design in parent.jobs(previous, data, identity['parent_identity']):
        ...
        for event in cfg['events']:
            ...
            control = parent.checked(key+'_hurdle', identity['parent_identity'])
...
def replay(ctx):
    ...
    for name, candidate, fold, seed, design in parent.jobs(previous, data, identity['parent_identity']):
        ...
        for event in cfg['events']:
            ...
            ref = parent.checked(key+'_hurdle', identity['parent_identity'])
...
def jobs(ctx):
    ...
    for name, candidate, fold, seed, design in parent.jobs(previous, data, identity['parent_identity']):
        ...
        for event in cfg['events']:
            key = name+'_'+event
            # Internal helper aliases: product=original hurdle, hurdle=rank-augmented.
            risks = dict(product_mse=parent.old.scores(parent.checked(key+'_hurdle', identity['parent_identity']), ids),
...
def decide(ctx):
    ...
    prior = json.loads((parent.PUBLIC/'analysis.json').read_text())
    records = []
    for key, _, _, _, design, utility, moving, risks in jobs(ctx):
        ...
        if array_hash(ids, choices['product_original']) != prior['views'][key+'_hurdle']['decision_sha256']:
...
def evaluate(ctx, *, verify):
        ...
        for arm, use in bits.items():
            ...
            if arm == 'product_original':
                for field in ('ADE_vs_CV', 'FDE_vs_CV', 'switch_rate', 'zero_CV', 'decision_sha256'):
                    if row[field] != prior['views'][key+'_hurdle'][field]:
    ...
    assert_identity(identity)
    result = dict(identity=identity, result_source='fresh_run_36_rank_augmented_torch_heads_and_metrics',
...
def main():
    ...
    for flag in ('prepare', 'pilot', 'train', 'resume', 'replay', 'decide', 'evaluate', 'verify'):
