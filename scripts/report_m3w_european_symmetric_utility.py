"""Nonselective reporting and independent accounting for symmetric utility."""
import json
...
import numpy as np
from scripts.report_m3w_european_cv_reference import require_verification, sha, value, ci
...
def accounting(r):
    ...
    if len(replay['checks'])!=18 or not all(c['paired_sampler_exact'] and c['total_draws']==512000
    ...
    parent = r['identity']['previous_identity']['previous_identity']['previous_identity']['previous_identity']['parent_identity']
    ...
    oldpath = ROOT/'outputs/publication_readiness_2026_09/european_protected_motion_v1/analysis.json'
    ...
    for key,artifacts in r['frozen_controls'].items():
        if artifacts!=old['training'][key]['artifacts']:
    ...
    for a in r['controls']:
        ...
        utility = load_artifact(r['training'][key]['artifacts']['scores'])
        risk_artifacts = r['frozen_controls'][f'{key}_{event}_{arm}']
        ...
        scale = r['training'][key]['lineage']['cost_scale']
    ...
    if len(checks)!=144 or len(r['frozen_controls'])!=90:
    ...
    return dict(result_source='fresh_accounting_on_cached_verified_models',analysis_sha256=sha(PUBLIC/'analysis.json'),
        reporter_sha256=sha(Path(__file__)),decision_checks=checks,frozen_risk_heads=72,
        frozen_utility_controls=18,matched_new_checkpoints=18,all_passed=True)


def summary(r):
    return dict(result_source=r['result_source'],analysis_sha256=sha(PUBLIC/'analysis.json'),
        source_rows=r['source_rows'],joint_rows=r['joint_rows'],new_heads=18,new_neural_updates=36000,
        ...
        cost_prediction=r['cost_prediction'],decision_changes=r['decision_changes'],
...
def main():
    ...
    lines = ['# Symmetric Utility Results','',
    ...
    for title,field in [('New Utility vs Frozen Old Utility','symmetric_vs_asymmetric'),('Neural vs Protected Damping','neural_vs_damping')]:
    ...
    lines = ['# Utility Training Losses','','Only the utility objective changed. These are minibatch fitting losses, not held-out accuracy.',
    ...
    lines += ['','The 100-update pilot is included in 36,000 updates. Every new head has exactly the old head\'s training draws, known-label support, preprocessing and initialization constants.',
    ...
    print(json.dumps(dict(all_passed=True,heads=18,policy_views=48,decision_receipts=144,
