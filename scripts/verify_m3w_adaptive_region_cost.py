"""Separate refresh, label, policy and metric checks for adaptive fit weighting."""
import json
from pathlib import Path
import sys
ROOT=Path(__file__).resolve().parents[1]
sys.path.insert(0,str(ROOT))
from scripts.run_m3w_adaptive_region_cost import load,load_states
...
def main():
    ...
    fit_rows=choices=refresh_checks=0
    for key,meta in views.items():
        ...
        cp=states[key]
        np.testing.assert_array_equal(cp['initial_loss_weights'],weight)
        assert [r['step'] for r in cp['refreshes']]==list(range(500,12000,500))
        for r in cp['refreshes']:
            model.load_state_dict(r['model']);rs=predict(model,x,d,pr,'bounded_native')
            rb=(d>0)&np.any(past[:,-1]!=past[:,-2],axis=1)&(rs[:,0]>rs[:,1])&(rs[:,1]<=.1*rs[:,0])
            np.testing.assert_array_equal(r['selected'],rb)
            rw=np.where(rb,4.,1.);rw/=np.dot(pr['weights'],rw);rw[~pr['known']]=0
            rw=rw.astype(np.float32)
            assert r['complete_selected']==int((rb&pr['known']).sum())
            assert np.isclose(np.dot(pr['weights'],rw),r['mean_weight'])
            refresh_checks+=1
        np.testing.assert_array_equal(cp['loss_weights'],rw)
    ...
    out=dict(analysis_sha256=file_digest(public/'analysis.json'),verifier_sha256=file_digest(Path(__file__)),
        ...
        supervision_rows_checked=fit_rows,initial_weight_vectors_checked=12,refreshes_recomputed=refresh_checks,
