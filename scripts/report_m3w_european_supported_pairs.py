"""Publish every registered supported-pair result and matched training diagnostic."""
import argparse
import csv
import json
...
from scripts import run_m3w_european_supported_pairs as run
...
def name(value):
    return value.replace('product', 'control').replace('hurdle', 'supported').replace('ranked', 'supported')
...
def reports(a):
    pub = run.PUBLIC
    rows = ['# Supported-Event Pairing: Every Registered View', '',
        'Control = frozen pair-then-filter rank head; supported = the same head with supported rows paired first.',
...
def training_diagnostics(a, training):
    ...
    for candidate in ('neural', 'damping097'):
        ...
        for event in ('all', 'easy'):
            ...
            old = [run.ranked.checked(k, a['identity']['ranked_identity'])['fit'] for k in keys]
    ...
    lines += ['', 'More supervised pairs or lower fitting loss is not proof of better excluded-locality ordering.',
...
def complete(a):
    ...
    for pid, v in last.items():
        ...
        if v['state'] != 'phase_complete' or 'run_m3w_european_supported_pairs.py' in process.stdout:
            raise ValueError('Training/evaluation phase is not terminal')
    parent = json.loads((run.ranked.PUBLIC/'completion_checks.json').read_text())
    tests = parent['scoped_test_files'] + ['tests/test_m3w_supported_rank_pairs.py',
                                          'tests/test_m3w_ratio_ranking_estimand.py']
    ...
    if result.returncode or not re.search(r'\b247 passed\b', result.stdout):
        raise ValueError('Scoped tests failed')
    run.json_write(run.PUBLIC/'completion_checks.json', dict(result_source='fresh_run_supported_event_pairing_training_and_verification',
        ...
        counterexample_sha256=run.digest(run.PUBLIC/'estimand_counterexample.json'),
        tests_sha256=run.digest(run.PUBLIC/'scoped_tests.txt'), scoped_test_files=tests, tests_passed=247,
    ...
    print(json.dumps(dict(heads=36, updates=72000, views=216, tests=247)))
...
def figures(a):
    ...
    for ax, component, title in zip(axes, ('ranking_at_product_count', 'ranking_at_hurdle_count'),
            ('At frozen ranked-control common counts', 'At supported-pair common counts')):
        ...
        ax.set_xlabel('Supported-pair advantage (pp of CV-normalized ADE)')
    ...
    fig.suptitle('Does supported-event pairing improve ordering at fixed coverage?\n'
