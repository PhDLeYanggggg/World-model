"""Separate label/weight/policy/error arithmetic for the fixed region experiment."""
import json
from pathlib import Path
import sys
ROOT=Path(__file__).resolve().parents[1]
sys.path.insert(0,str(ROOT))
from scripts.run_m3w_conditional_cost import load,load_states
...
def main():
    ...
    for policy in policies:
        ...
        for a in ('tempered','native','fraction'):
            check_contrast(r['ADE']['by_scene'],prior['summaries'][a][policy]['ADE']['by_scene'],cfg['sites'],report['contrasts'][policy][a])
    p=report['summaries']['strict_stop'];c=report['contrasts']['strict_stop']['tempered']
