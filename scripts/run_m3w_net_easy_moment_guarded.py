...
from src.world_model.m3w_net_easy_risk import targets, moments
from src.world_model.m3w_net_easy_guard import allocate, validate_receipt, require_replay_outputs

CONFIG = 'configs/m3w_net_easy_moment_guarded_v1.json'
CODE = ('scripts/run_m3w_net_easy_moment_guarded.py', 'src/world_model/m3w_net_easy_risk.py',
        'src/world_model/m3w_net_easy_guard.py', 'tests/test_m3w_net_easy_guard.py',
...
def load():
    ...
    training_path = ROOT/cfg['training_output']/'identity.json'
    assert file_digest(training_path) == cfg['training_identity_sha256']
    fresh_training_identity = json.loads(training_path.read_text())
    assert_current(fresh_training_identity)
    bindings = dict(fresh_training_identity['source_bindings'])
    for p, h in allocation[-1]['source_bindings'].items():
        if p in bindings and bindings[p] != h:
            raise ValueError('Changed frozen allocation dependency')
        bindings[p] = h
    ...
    for p in (CONFIG, cfg['registration'], cfg['guard_registration'], str(training_path.relative_to(ROOT)), diagnostic, *CODE,
...
def trial(cfg, key, action):
    return ROOT/cfg['training_output']/'trials'/key/action


def receipts(pack):
    cfg, ap, tp, old, identity = pack
    ish = cfg['training_identity_sha256']; result = {}
    for key in ap[4]:
        for action in cfg['actions']:
            ...
            validate_receipt(r, key, action, ish, tp[7][key, action], cp)
            assert cp['settings'] == cfg['forest']
            result[key, action] = r
    return result
...
def evaluate(pack, args, beat):
    ...
    if args.verify:
        require_replay_outputs(public, root, cfg['actions'], cfg['seeds'])
