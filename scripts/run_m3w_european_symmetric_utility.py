"""Source-only paired utility-loss ablation with frozen forecasts and risk heads."""
import argparse
import fcntl
import json
import os
...
from src.evaluation.m3w_symmetric_utility import validate_config, complete_budget, verify_matched_checkpoint
...
PRIVATE = ROOT/'data/stage_cvpr2027_experiments/european_symmetric_utility_v1'
PUBLIC = ROOT/'outputs/publication_readiness_2026_09/european_symmetric_utility_v1'
CONFIG = 'configs/m3w_european_symmetric_utility_v1.json'
FILES = (CONFIG, 'scripts/run_m3w_european_symmetric_utility.py',
    'src/evaluation/m3w_symmetric_utility.py', 'tests/test_m3w_symmetric_utility.py',
    'tests/test_m3w_utility_objective_semantics.py',
    'outputs/publication_readiness_2026_09/european_symmetric_utility_v1/registration.md')
...
def assert_identity(identity):
    for path, sha in identity['bindings'].items():
        if digest(ROOT/path) != sha:
            raise ValueError('Symmetric-utility registration changed: '+path)
...
def load():
    ...
    for name in ('verification.json', 'checkpoint_replay.json', 'accounting_audit.json'):
        r = json.loads((prior.PUBLIC/name).read_text())
        if r['analysis_sha256'] != digest(prior.PUBLIC/'analysis.json') or not r.get('all_passed', True):
    ...
    immutable_json(PUBLIC/'matrix.json', dict(identity=identity, new_heads=18,
        frozen_risk_heads=72, cached_control_utility_heads=18, new_neural_updates=36000,
...
def checked_head(candidate, key, identity):
    r = json.loads((PRIVATE/'heads'/f'{candidate}_{key}'/'complete.json').read_text())
    if r['identity']['identity'] != identity or not prior.previous.prior.previous.artifacts_ok(r):
        raise ValueError('Changed or incomplete symmetric utility head')
...
def train(reg, data, identity, designs, *, resume, pilot):
    for key, design, ti in designs:
        ...
        for candidate in reg['candidates']:
            a = prior.assemble(candidate, data, identity['previous_identity'], key, design, ti)
            y, pr, lineage = prior.task_data(a, data, design, 'utility')
            old = prior.checked_head(key, 'utility', 'neural_underharm4', candidate, identity['previous_identity'])
            fit, held = design['train_ids'], design['held_ids']
            name = f'{candidate}_{key}'
            directory = PRIVATE/'heads'/name
            hid = dict(identity=identity, lineage=lineage, arm='mse', candidate=candidate,
                old_checkpoint=old['artifacts']['checkpoint'])
            if (directory/'complete.json').exists():
                if checked_head(candidate,key,identity)['identity'] != hid:
                    raise ValueError('Completed utility lineage changed')
                beat('verified_completed_head', trial=name)
                continue
            start = time.monotonic()
            beat('utility_training', trial=name, train_rows=len(fit), held_rows=len(held))
            model, report = fit_neural(a['x'][fit], y, data['sites'][fit], a['same'][fit], pr,
            ...
            values = np.maximum(predict_neural(model, a['x'][held], a['same'][held], pr), 0)
            path = directory/'scores.npz'
            prior.previous.prior.atomic_arrays(path, ids=held, costs=values)
            ...
            assert_identity(identity)
            beat('utility_complete', trial=name, updates=report['step'])
            del a, state, oldstate, model


def decisions(reg, data, a, held, qmask, utility, moments, name, identity, support, guard, verify):
    ...
    out, queries = candidate_decisions(reg,data,a,held,qmask,utility,moments,available)
...
def evaluate(reg, data, identity, designs, qmask, verify):
    heads = {f'{candidate}_{key}':checked_head(candidate,key,identity) for candidate in reg['candidates']
        for key,_,ti in designs if ti['kind']=='complement'}
    updates = complete_budget(heads)
    oldheads = {f'{candidate}_{key}_{task}_{arm}':prior.checked_head(key,task,arm,candidate,identity['previous_identity'])
        for candidate in reg['candidates'] for key,_,ti in designs if ti['kind']=='complement'
        for task,arm in prior.specs(reg)}
    n = len(data['sites'])
    ...
    for key,design,ti in designs:
        ...
        for candidate in reg['candidates']:
            ...
            utility_r = heads[f'{candidate}_{key}']
            cost = prior.scores(utility_r,'utility',held)
            u = (cost[:,0]-cost[:,1])/utility_r['identity']['lineage']['cost_scale']
            oldcost = prior.scores(oldheads[f'{candidate}_{key}_utility_neural_underharm4'],'utility',held)
            truth = prior.paired_cost_labels(cv[held],s['ade'][held])
            for site in sorted(set(data['sites'][held])):
                mask = (data['sites'][held]==site)&np.isfinite(truth).all(1)
                cost_metrics[f'{candidate}_{key}_{site}'] = {label:dict(rows=int(mask.sum()),
                    mean_absolute_error=np.abs(c[mask]-truth[mask]).mean(0).tolist(),
                    bias=(c[mask]-truth[mask]).mean(0).tolist(),
                    positive_utility_fraction=float((c[mask,0]>c[mask,1]).mean()))
                    for label,c in [('mse',cost),('underharm4',oldcost)]}
            support = utility_r['identity']['lineage']['source_support']
            for event in reg['events']:
                for arm in reg['arms']:
                    moments = prior.scores(oldheads[f'{candidate}_{key}_{event}_{arm}'],event,held)
                    for guard in reg['support_controls']:
                        ...
                        oldpath = prior.PRIVATE/'decisions'/(name+'.npz')
    ...
    result = dict(result_source='fresh_run_symmetric_utility_training_and_decisions',identity=identity,
        ...
        frozen_controls={k:r['artifacts'] for k,r in oldheads.items()},controls=receipts,
        source_rows=n,joint_rows=int(qmask.sum()),policies=policies,neural_vs_damping=contrasts,
        symmetric_vs_asymmetric=old_contrasts,decision_changes=changes,cost_prediction=cost_metrics,
        new_heads=18,cached_heads=90,new_neural_updates=updates,new_forecaster_training=False,
...
def replay(reg,data,identity,designs):
    ...
    for key,design,ti in designs:
        if ti['kind']!='complement':
            continue
        for candidate in reg['candidates']:
            r = checked_head(candidate,key,identity)
            old = prior.checked_head(key,'utility','neural_underharm4',candidate,identity['previous_identity'])
            ...
            a = prior.assemble(candidate,data,identity['previous_identity'],key,design,ti)
            _,pr,lineage = prior.task_data(a,data,design,'utility')
            ...
            fresh = np.maximum(predict_neural(model,a['x'][held],a['same'][held],pr),0)
            saved = prior.scores(r,'utility',design['held_ids'])[:len(held)]
            np.testing.assert_array_equal(fresh,saved)
            checks.append(dict(trial=f'{candidate}_{key}',rows=len(held),exact=True,max_difference=0.,
            ...
            del a,cp,oldcp,model
    if len(checks)!=18:
        raise ValueError('All eighteen new heads required')
    assert_identity(identity)
