"""Separate cost/BCE traces and every paired cost contrast."""
import json
...
def main():
    ...
    with plt.rc_context({'font.family':'DejaVu Sans','font.size':10,'svg.fonttype':'none','svg.hashsalt':'m3w-membership-auxiliary-v1'}):
        fig,axes=plt.subplots(2,2,figsize=(11,7))
        for j,arm in enumerate(('cost_only','membership_aux')):
            for i,key in enumerate(('cost_loss','membership_BCE')):
                ax=axes[i,j]
                for pair,color in (('full','#1d718f'),('motion_only','#b63e55')):
                    ...
                    vs=np.array([[{v['step']:v[key] for v in t}[s] for s in steps] for t in ts])
                ...
                ax.set_title(arm+' / '+key); ax.set_xlabel('Updates'); ax.set_ylabel('Fixed-batch component loss')
                ax.grid(alpha=.2); ax.spines[['top','right']].set_visible(False)
        axes[0,0].legend(frameon=False)
        fig.suptitle('Median and IQR / cost and auxiliary losses shown separately')
        ...
        for ax,comp in zip(axes,('aux_vs_control','aux_vs_original','aux_vs_conditional')):
            ...
            for pair in ('full','motion_only'):
                for role,ms in sorted(cs[pair].items()):
                    ...
                    i+=1
            ax.set_title(comp.replace('aux_vs_','Auxiliary vs ')); ax.axvline(0,color='#888',linewidth=.8)
        ...
        fig.suptitle('Auxiliary supervision / positive disagreement / 3,000 locality resamples')
