"""Two ordered cross-moment controls; fit both before any new readout."""
import argparse
import fcntl
import json
import os
...
from scripts import run_m3w_european_supported_pairs as supported
from scripts.report_m3w_european_supported_pairs import verify as verified_supported
...
from src.world_model.m3w_cross_moment_rank import fit, ranking_loss
from src.world_model.m3w_supported_rank_pairs import ranking_loss as old_ranking_loss
...
PUBLIC_ROOT = ROOT/'outputs/publication_readiness_2026_09/european_cross_moment_v1'
PRIVATE_ROOT = ROOT/'data/stage_cvpr2027_experiments/european_cross_moment_v1'
MODE = None
PUBLIC = PRIVATE = None
CONFIG = 'configs/m3w_european_cross_moment_v1.json'
FILES = (CONFIG, 'scripts/run_m3w_european_cross_moment.py', 'src/world_model/m3w_cross_moment_rank.py',
         'tests/test_m3w_cross_moment_rank.py', 'tests/test_m3w_cross_moment_protocol.py',
         'outputs/publication_readiness_2026_09/european_cross_moment_v1/registration.md')


def configure(mode):
    global MODE, PUBLIC, PRIVATE
    if mode not in ('batch', 'fitting'):
        raise ValueError('Explicit batch or fitting normalizer required')
    MODE = mode
    PUBLIC, PRIVATE = PUBLIC_ROOT/mode, PRIVATE_ROOT/mode
...
def assert_identity(identity):
    if identity['mode'] not in ('batch', 'fitting'):
        raise ValueError('Unknown registered mode')
    if identity['mode'] == 'fitting':
        control = identity['control_identity']
        if (control['mode'] != 'batch' or control['bindings'] != identity['bindings']
                or control['supported_identity'] != identity['supported_identity']):
            raise ValueError('Fixed-scale control must be this registered batch-scale experiment')
    coverage.assert_identity(identity['coverage_identity'])
    supported.assert_identity(identity['supported_identity'])
    if digest(supported.PUBLIC/'analysis.json') != identity['supported_analysis_sha256']:
...
def load():
    ...
    supported_prior = verified_supported()
    if supported_prior['identity']['parent_identity'] != pid:
        raise ValueError('Ranked control producer lineage differs')
    if cfg['prior_supported_analysis_sha256'] != digest(supported.PUBLIC/'analysis.json'):
    ...
    identity = dict(mode=MODE, parent_identity=pid, coverage_identity=prior['identity'],
        coverage_analysis_sha256=digest(coverage.PUBLIC/'analysis.json'),
        supported_identity=supported_prior['identity'], supported_analysis_sha256=digest(supported.PUBLIC/'analysis.json'),
        bindings={f: digest(ROOT/f) for f in FILES})
    if MODE == 'fitting':
        identity['control_identity'] = json.loads((PRIVATE_ROOT/'batch/identity.json').read_text())
        if identity['control_identity']['supported_identity'] != identity['supported_identity']:
            raise ValueError('Mismatched sequential control lineage')
...
def checked(key, identity):
    r = json.loads((PRIVATE_ROOT/identity['mode']/'heads'/key/'complete.json').read_text())
    ...
    if digest(PUBLIC_ROOT/identity['mode']/'fitting_support.json') != r['identity']['normalization_audit_sha256']:
        raise ValueError('Fitting-only normalization audit changed')
...
def control_checked(key, identity):
    if identity['mode'] == 'batch':
        return supported.checked(key, identity['supported_identity'])
    return checked(key, identity['control_identity'])


def control_analysis_path(identity):
    return supported.PUBLIC/'analysis.json' if identity['mode'] == 'batch' else PUBLIC_ROOT/'batch/analysis.json'


def expected_control_choice(key, identity):
    if identity['mode'] == 'batch':
        old = json.loads(control_analysis_path(identity).read_text())
        return old['views'][key+'_hurdle_original']['decision_sha256']
    refs = read_decisions(identity['control_identity'])
    with np.load(ROOT/refs[key]['path'], allow_pickle=False) as z:
        return array_hash(z['ids'], z['hurdle_original'])


def ensure_all_decisions():
    for mode in ('batch', 'fitting'):
        identity = json.loads((PRIVATE_ROOT/mode/'identity.json').read_text())
        if identity['mode'] != mode:
            raise ValueError('Wrong decision-bank mode')
        assert_identity(identity)
        read_decisions(identity)
        replay = json.loads((PUBLIC_ROOT/mode/'head_replay.json').read_text())
        if replay['identity'] != identity or not replay['all_passed'] or len(replay['checks']) != 36:
            raise ValueError('Both modes must replay before any readout')
...
def train(ctx, *, pilot, resume):
    ...
    for name, candidate, fold, seed, design in parent.jobs(previous, data, identity['parent_identity']):
        ...
        for event in cfg['events']:
            ...
            control = control_checked(key, identity)
            ...
            audit = json.loads((PUBLIC/'fitting_support.json').read_text())
            if audit['identity'] != identity or not audit['fitting_only']:
                raise ValueError('Fitting-only normalizer provenance required')
            fixed_scale = audit['rows'][key]['mean_cross_weight_sum'] if MODE == 'fitting' else None
            hid = dict(normalization_audit_sha256=digest(PUBLIC/'fitting_support.json'), experiment=identity, lineage=a['lineage'], event=event, target_sha256=array_hash(y),
...
def replay(ctx):
    ...
    for name, candidate, fold, seed, design in parent.jobs(previous, data, identity['parent_identity']):
        ...
        for event in cfg['events']:
            ...
            ref = control_checked(key, identity)
            ...
            audit = json.loads((PUBLIC/'fitting_support.json').read_text())
            expected_scale = audit['rows'][key]['mean_cross_weight_sum'] if MODE == 'fitting' else None
            if state['fixed_denominator'] != expected_scale or r['fit']['fixed_denominator'] != expected_scale:
                raise ValueError('Saved fixed normalizer disagrees with fitting audit')
...
def jobs(ctx):
    ...
    for name, candidate, fold, seed, design in parent.jobs(previous, data, identity['parent_identity']):
        ...
        for event in cfg['events']:
            key = name+'_'+event
            # Both helpers stay frozen; product=preceding control, hurdle=current mode.
            risks = dict(product_mse=parent.old.scores(control_checked(key, identity), ids),
...
def decide(ctx):
    ...
    for key, _, _, _, design, utility, moving, risks in jobs(ctx):
        ...
        if array_hash(ids, choices['product_original']) != expected_control_choice(key, identity):
...
def read_decisions(identity):
    r = json.loads((PRIVATE_ROOT/identity['mode']/'decisions_complete.json').read_text())
    if r['identity'] != identity or len(r['archives']) != 36 or len({v['group'] for v in r['archives']}) != 36:
...
def evaluate(ctx, *, verify):
    cfg, _, data, identity, _ = ctx
    ensure_all_decisions()
    archives = read_decisions(identity)
    prior = json.loads(control_analysis_path(identity).read_text())
    ...
    assert_identity(identity)
    result = dict(identity=identity, result_source='fresh_run_36_cross_moment_'+MODE+'_heads_and_metrics',
        internal_helper_aliases=cfg['internal_helper_aliases'][MODE], arm_names=ARM_NAMES,
        control_analysis_sha256=digest(control_analysis_path(identity)),
        ...
        for event in cfg['events']:
            ...
            for _ in range(cfg['support_audit_batches']):
                ...
                stats.append(dict(old_pairs=old['rank_pairs'], new_pairs=new['rank_pairs'],
                                  ...
                                  weight_sum=new['rank_weight_sum'], max_weight_fraction=new['rank_max_weight_fraction'],
                                  effective_pairs=new['rank_effective_pairs']))
            scale = float(np.mean([r['weight_sum'] for r in stats]))
            if not np.isfinite(scale) or scale <= 0:
                raise ValueError('No finite supported fixed normalizer; preserve audit as blocker')
            rows[name+'_'+event] = dict(batches=len(stats), mean_cross_weight_sum=scale,
                cross_weight_sum_quantiles=np.quantile([r['weight_sum'] for r in stats], [.5, .95, .99, 1]).tolist(),
                max_pair_weight_fraction=max(r['max_weight_fraction'] for r in stats),
                mean_effective_pairs=float(np.mean([r['effective_pairs'] for r in stats])),
...
def main():
    ...
    parser.add_argument('--normalizer', choices=('batch', 'fitting'), required=True)
    for flag in ('prepare', 'audit', 'pilot', 'train', 'resume', 'replay', 'decide', 'evaluate', 'verify'):
        parser.add_argument('--'+flag, action='store_true')
    args = parser.parse_args()
    if not any(v for k, v in vars(args).items() if k != 'normalizer') or (args.pilot and (args.evaluate or args.verify or args.decide)):
        parser.error('Explicit phases; no pilot outcome readout')
    configure(args.normalizer)
