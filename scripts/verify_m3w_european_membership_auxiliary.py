"""Replay causal auxiliary costs, fitting diagnostics and held metrics."""
import inspect
import json
...
from scripts import run_m3w_european_membership_auxiliary as run
from scripts.report_m3w_european_membership_auxiliary import aggregates
...
def replay(cfg,identity):
    ...
    for g,data,pairs in run.base.contexts(identity['source']):
        ...
        for pair in cfg['pairs']:
            ...
            for held in sorted(set(sites)):
                ...
                easy=run.parent.parent.method.labels(cv[tr],cut); labels=run.parent.parent.method.labels(cv[te],cut)
                target=run.tail.diagnostic.event_targets(by[te],cv[te],cut)
                tag=name+'_'+pair+'_'+held; tref,href,oldstate,_=run.parent.reference(tag,x,tr,te,pr,env,bi)
                ...
                for arm in ('original_mean','conditional'): assert f['metrics'][arm]==oldf['metrics'][arm]
                for arm in cfg['arms']:
                    ...
                    assert not inp['held_targets_used_for_fit'] and not inp['membership_probability_multiplied_into_cost']
                    assert s['step']==2000 and r['fit']['parameters']==24901
                    ...
                    with np.load(directory/'scores.npz',allow_pickle=False) as z:
                        np.testing.assert_array_equal(z['ids'],bi[te]); pred=z['scores'].copy(); p=z['membership'].copy()
                    n=min(4096,te.sum()); score,prob=run.method.predict(model,x[te][:n],env[te][:n],pr)
                    np.testing.assert_array_equal(score[:,[1,3]],pred[:n][:,[1,3]]); np.testing.assert_array_equal(prob,p[:n])
                    np.testing.assert_array_equal(pred[:,[0,2]],href[:,[0,2]])
                    fitting,_=run.method.predict(model,x[tr],env[tr],pr); fitting=run.parent.compose(fitting[:,[1,3]],tref)
                    edges=run.tail.training_edges(fitting,env[tr],pr,cfg)
                    assert edges==f['training'][arm]['edges']
                    assert run.tail.diagnostic.summarize(fitting,y,env[tr],sites[tr],edges)==f['training'][arm]['training']
                    assert run.parent.measure(pred,target,env[te],sites[te],edges)==f['metrics'][arm]
                    assert run.parent.parent.measure(p,labels,env[te],sites[te])==f['membership'][arm]
                    if arm=='cost_only':
                        error=float(np.max(np.abs(pred-href))); assert error==f['control_prediction_max_abs_error']; maximum=max(maximum,error)
                    for subset,mask in (('all',np.ones(te.sum(),bool)),('envelope_positive',env[te]>0)):
                        ...
                        pp,tt=pred[known],target[known]; m=f['metrics'][arm][subset]
    ...
    r=dict(identity=identity,source_binding=binding,training=run.artifact(run.PRIVATE/'training_complete.json'),
        records=records,independent_cost_rank_checks=checks,control_prediction_max_abs_error=maximum,all_passed=True)
...
def main():
    ...
    old=json.loads((run.attribution.PUBLIC/'verification.json').read_text())
    tests=sorted(set(old['test_files'])|{'tests/test_m3w_membership_auxiliary.py','tests/test_m3w_membership_auxiliary_reporting.py'})
    ...
    (run.PRIVATE/'tests.log').write_text(p.stdout+'\n'+p.stderr); print(p.stdout[-1600:],flush=True); assert p.returncode==0
    ...
    figures={p:run.digest(run.PUBLIC/p) for p in ('training_loss.svg','paired_contrasts.svg')}
    subprocess.run([sys.executable,'scripts/plot_m3w_european_membership_auxiliary.py'],cwd=ROOT,check=True)
    assert figures=={p:run.digest(run.PUBLIC/p) for p in figures}
    ...
    bindings=sorted(set(run.FILES+tests+['scripts/plot_m3w_european_membership_auxiliary.py',str(Path(__file__).relative_to(ROOT))]))
    run.immutable_json(run.PUBLIC/'verification.json',dict(all_passed=True,artifacts=artifacts,
        ...
        checkpoint_prefix_replays=len(receipt['records']),independent_cost_rank_checks=receipt['independent_cost_rank_checks'],
        deterministic_figures=True,policy_changed=False,independent_confirmation=False))
    print(json.dumps(dict(tests=count,files=len(tests),all_passed=True,public_artifacts=len(artifacts),source_bindings=len(bindings))))
