"""Adaptive fit-region weighting; fixed forecasts, draws, thresholds and roles."""
import argparse
import fcntl
import json
import os
...
from src.world_model.m3w_adaptive_region_cost_head import fit
from src.evaluation.m3w_conditional_cost_audit import strict_bits
from src.evaluation.m3w_experiment_contract import file_digest
import numpy as np
import torch

CONFIG = 'configs/m3w_adaptive_region_cost_v1.json'
CODE = ('scripts/run_m3w_adaptive_region_cost.py', 'scripts/verify_m3w_adaptive_region_cost.py',
        'src/world_model/m3w_adaptive_region_cost_head.py', 'src/evaluation/m3w_adaptive_region_cost_eval.py',
        'tests/test_m3w_adaptive_region_cost.py')
...
def validate_config(cfg, pcfg):
    if (cfg['sites'] != pcfg['sites'] or cfg['seeds'] != pcfg['seeds']
            ...
            or cfg['primary_reference'] != 'frozen_region_strict_stop' or cfg['refresh_every'] != 500
            or cfg['policies'] != pcfg['policies']
        ...
        raise ValueError('Only fit-region refresh every 500 updates is allowed')


def load():
    cfg = json.loads((ROOT/CONFIG).read_text())
    pcfg, data, views, predictions, earlier, refs, pid = parent.load()
    records, states = parent.load_states(pcfg, views, pid)
    validate_config(cfg, pcfg)
    prior = json.loads((ROOT/cfg['parent_analysis']).read_text())
    assert prior['identity'] == pid and prior['new_updates'] == 144000
    ...
    refs = refs | {(key, 'frozen_region'): cp for key, cp in states.items()}
    prior = dict(archives=prior['archives'], summaries=dict(frozen_region=prior['summaries'],
        **earlier['summaries']))
...
def check_state(cp, reference, w, cfg):
    ...
    np.testing.assert_array_equal(cp['initial_loss_weights'], w)
    assert cp['refresh_every'] == cfg['refresh_every'] and cp['region_multiplier'] == cfg['region_multiplier']
    assert [r['step'] for r in cp['refreshes']] == list(range(500, 12000, 500))
    assert all(np.isclose(r['mean_weight'], 1.) for r in cp['refreshes'])
    final = region_weights(cp['refreshes'][-1]['selected'], cp['preprocess'], cfg['region_multiplier'])
    np.testing.assert_array_equal(cp['loss_weights'], final)
...
def train(cfg, data, views, references, identity, args, beat):
    for key, meta in views.items():
        ...
        _, result = fit(x,y,d,past,data['sites'][ids],pr,w,seed=meta['seed'],settings=cfg['training'],
            refresh_every=cfg['refresh_every'],multiplier=cfg['region_multiplier'],
...
def main():
    ...
    with (root/'runner.lock').open('a') as lock:
        ...
        if args.audit_only:beat(state='preflight_pass',bindings=len(identity['source_bindings']),new_fits=12)
            ...
            from src.evaluation.m3w_adaptive_region_cost_eval import evaluate
