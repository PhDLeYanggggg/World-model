"""Fixed matched observed-motion forecasts, no sealed-role evaluation or tuning."""
...
from src.world_model.m3w_observed_motion import VARIANTS, feature_variant
...
def summarize(trials, reg):
    ...
    for objective in reg['objectives']:
        ...
        mag = matrix('magnitude', objective, 'primary_ADE')
        for variant in VARIANTS:
            ...
            summary[variant + '_' + objective] = dict(
                ...
                vs_magnitude=paired_interval(pred, mag, reg['bootstrap_resamples']),
...
def main():
    ...
    if reg['variants'] != list(VARIANTS) or reg['objectives'] != ['row_log', 'scene_ade_harm']:
    ...
    lines = ['# Past Image Motion Forecasting', '', 'Fit-only exploratory comparison; no independent confirmation.', '',
