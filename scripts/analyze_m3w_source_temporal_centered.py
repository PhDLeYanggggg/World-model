"""Report every fixed centering arm and cached matched control, without selection."""
import argparse
import json
from pathlib import Path
import sys
ROOT=Path(__file__).resolve().parents[1];sys.path.insert(0,str(ROOT))
from scripts.run_m3w_source_temporal_centered import load_config,setup,immutable_json
...
def main():
    ...
    means={};summary={}
    for arm in ('geometry','current','sequence','centered','centered_unit'):
        source=control if arm in ('geometry','current','sequence') else report
        errors=[];fdes=[]
        for seed in reg['seeds']:
            item=next(v for v in source['oof_labels'] if v['arm']==arm and v['seed']==seed)
        ...
        for site in reg['sites']:
            m=sites==site;fits=[t for t in source['trials'] if t['arm']==arm and t['site']==site]
        ...
        summary[arm]=dict(result_source='cached_verified_recomputed' if source is control else 'fresh_run',
            ...
            sites=details,positive_held_models=sum(t['held']['gain_percent']>0 for t in source['trials'] if t['arm']==arm))
    previous=json.loads((ROOT/parent['reports']/'analysis.json').read_text())
    for arm in ('geometry','current','sequence'):
    ...
    contrasts=[]
    for a,b in [('centered','sequence'),('centered_unit','sequence'),('centered','geometry'),
                ('centered_unit','geometry'),('centered_unit','centered')]:
        ...
        contrasts.append(dict(candidate=a,reference=b,gain_difference_pp=float(100*diff.mean()/cv_site.mean()),
            conditional_four_site_ci95=np.quantile(values,[.025,.975]).tolist()))
    result=dict(result_source='fresh_run_centered_heads_cached_verified_original_controls',
        new_models=24,new_updates=240000,cached_control_models=36,rows=len(ids),sites=4,recordings=29,
