...
def main():
    ...
    rows=[(event,arm,guard) for event in ('all','easy') for arm in ('ridge','neural_mse')
    ...
    axes[0].set_yticks(np.arange(len(rows)),[
        ...
        for e,a,g in rows])
    axes[0].invert_yaxis()
    for ax,title in zip(axes,['Neural: new risk study vs old risk','Neural vs equally protected damping',
    ...
    fig.suptitle('Symmetric risk, frozen utility: all eight neural configurations and three seeds\n'
        '3,000 conditional source-locality bootstrap resamples; x marker means a zero-CV case was harmed',fontsize=12)
    svg = PUBLIC/'risk_ablation.svg'
    fig.savefig(svg)
    svg.write_text('\n'.join(line.rstrip() for line in svg.read_text().splitlines())+'\n')
    fig.savefig(PUBLIC/'risk_ablation.png',dpi=160)
    plt.close(fig)
