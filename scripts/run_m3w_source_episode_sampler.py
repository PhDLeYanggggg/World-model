"""Fixed equal-episode training exposure, with unchanged source-held evaluation."""
import argparse
import json
import os
...
def load_config(path):
    ...
    if (str(path)!=reg['registration_path'] or reg['role']!='training_only_equal_episode_exposure'
        ...
        raise ValueError('Fixed source-only episode-sampling contract required')
...
def main():
    ...
    with np.load(ep,allow_pickle=False) as a:
        episode_ids=a['ids'].copy(); episode_keys=a['episodes'].copy()
    np.testing.assert_array_equal(episode_ids,cached.ids)
    ...
    identity=dict(registration_sha256=file_digest(args.registration),data=data.identity,assignment=data.assignment_hash,
        ...
        episode_mapping_sha256=file_digest(ep),
    ...
    for site in reg['sites']:
        ...
        weights,weight_summary=episode_weights(train,episode_ids,episode_keys)
        sampler_sha=array_hash(train,weights)
        tr_pos=np.searchsorted(episode_ids,train); ho_pos=np.searchsorted(episode_ids,held)
        assert not set(episode_keys[tr_pos]) & set(episode_keys[ho_pos])
        ...
        checks.append(dict(site=site,fold=fold,target_poison_rows=8,
                           sampler_sha256=sampler_sha,**weight_summary))
        for seed in reg['seeds']:
            draw_rng=torch.Generator().manual_seed(seed+7919)
            expected_draws=np.zeros(len(train),np.int64)
            for _ in range(reg['training']['updates']):
                local=torch.multinomial(torch.as_tensor(weights,dtype=torch.float64),
                    reg['training']['batch_size'],replacement=True,generator=draw_rng).numpy()
                np.add.at(expected_draws,local,1)
            for arm in ARMS:
                ...
                assert torch.equal(saved['sampler_rng'],draw_rng.get_state())
                np.testing.assert_array_equal(saved['train_ids'],train)
                np.testing.assert_array_equal(saved['draw_counts'],expected_draws)
                ...
                trial=dict(identity=ti,trial=key,site=site,seed=seed,arm=arm,fit=result,
                    result_source='fresh_run_torch_episode_balanced_exposure',parameters=sum(v.numel() for v in model.parameters()),
                    ...
                    prediction_path=str(pp.relative_to(ROOT)),prediction_sha256=file_digest(pp),weighted_sampler_recomputed=True,
                    sampled_draws_sha256=array_hash(saved['draw_counts']))
